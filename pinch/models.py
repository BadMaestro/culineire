...
def unique_media_folder_for_pinch(item) -> str:
    if getattr(item, "media_folder", None):
        return item.media_folder

    base_name = safe_media_segment(getattr(item, "title", ""), "pinch")
    author = getattr(item, "author", None)
    existing = Pinch.objects.exclude(pk=item.pk)
...
def pinch_base_folder(item) -> str:
    author_folder = safe_author_folder(getattr(item, "author", None))
    item_folder = unique_media_folder_for_pinch(item)
    return f"pinch/{author_folder}/{item_folder}"


def pinch_cover_upload_to(instance, filename: str) -> str:
    extension = Path(filename).suffix.lower() or ".jpg"
    return f"{pinch_base_folder(instance)}/cover-{uuid4().hex[:12]}{extension}"


def pinch_gallery_upload_to(instance, filename: str) -> str:
    extension = Path(filename).suffix.lower() or ".jpg"
    sort_order = instance.sort_order or 1
    return f"{pinch_base_folder(instance.pinch_item)}/gallery/img{sort_order}-{uuid4().hex[:12]}{extension}"
...
class Pinch(models.Model):
    ...
    cover_image = models.ImageField(
        upload_to=pinch_cover_upload_to,
        blank=True,
        null=True,
    ...
    saves = GenericRelation(SavedContent, related_query_name="pinch_items")
    reactions = GenericRelation(ContentReaction, related_query_name="pinch_items")
    ...
    def get_absolute_url(self):
        return reverse("pinch:detail", kwargs={"slug": self.slug})
    ...
    def generate_unique_slug(self) -> str:
        base_slug = slugify(self.title)[:200] or "pinch"
        slug = base_slug
        counter = 2
        while Pinch.objects.exclude(pk=self.pk).filter(slug=slug).exists():
            suffix = f"-{counter}"
    ...
    def generate_emoji_description(self) -> None:
        ...
        try:
            ...
            logger.warning("Pinch.generate_emoji_description failed: %s", exc)

    def save(self, *args, **kwargs):
        if not self.slug:
            self.slug = self.generate_unique_slug()
        if not self.media_folder:
            self.media_folder = unique_media_folder_for_pinch(self)
...
class PinchGalleryImage(models.Model):
    ...
    image = models.ImageField(upload_to=pinch_gallery_upload_to, validators=[validate_image_upload])
...
class PinchComment(models.Model):
    """A user comment on an Pinch item.
