...
from .forms import PinchAuthoringForm
from .models import Pinch, PinchComment, PinchGalleryImage
from .telegram_preview import get_telegram_preview_meta
from .visibility import can_view_pinch_public_area


def _require_public_area_access(request):
    if not can_view_pinch_public_area(request.user):
        raise Http404
...
def _initial_status_for_author(author):
    if author_skips_approval(author):
        return Pinch.Status.APPROVED
    return Pinch.Status.PENDING


def _initial_status_fields(author, actor):
    status = _initial_status_for_author(author)
    fields = {"status": status}
    if status == Pinch.Status.APPROVED:
        now = timezone.now()
...
def _created_message(item, pending_message):
    if item.status == Pinch.Status.APPROVED:
        return f'Pinch "{item.title}" approved and is now live.'
    return pending_message


def _public_queryset(approved_only=True):
    queryset = Pinch.objects.all()
    if approved_only:
        queryset = queryset.filter(status=Pinch.Status.APPROVED)
    ...
    return (
        ...
        .prefetch_related(
            Prefetch(
                "gallery_images",
                queryset=PinchGalleryImage.objects.filter(is_active=True).order_by("sort_order", "id"),
...
def _user_state(queryset, user):
    ...
    content_type = ContentType.objects.get_for_model(Pinch)
...
def feed(request):
    ...
    return render(request, "pinch/feed.html", {
        "items": items,
        "content_type_choices": Pinch.ContentType.choices,
...
def detail(request, slug):
    if is_moderator(request.user):
        queryset = _public_queryset(approved_only=False).exclude(status=Pinch.Status.ARCHIVED)
    ...
    if item.status == Pinch.Status.APPROVED:
        if not can_view_pinch_public_area(request.user) and not can_preview:
            raise Http404
    elif not can_preview:
        raise Http404
    Pinch.objects.filter(pk=item.pk).update(view_count=F("view_count") + 1)
    ...
    return render(request, "pinch/detail.html", {
        "item": items[0],
        ...
        "can_edit": can_edit,
        "comments": comments,
        "can_view_ab_public": can_view_pinch_public_area(request.user),
    })


class PinchCreateView(AuthorRequiredMixin, CreateView):
    model = Pinch
    form_class = PinchAuthoringForm
    template_name = "pinch/form.html"
    success_url = reverse_lazy("pinch:feed")
...
class PinchUpdateView(AuthorRequiredMixin, UpdateView):
    model = Pinch
    form_class = PinchAuthoringForm
    template_name = "pinch/form.html"

    def get_queryset(self):
        return Pinch.objects.filter(author=self.author)
    ...
    def form_valid(self, form):
        ...
        if item.status == Pinch.Status.APPROVED and not is_moderator(self.request.user):
...
class PinchDeleteView(AuthorRequiredMixin, DeleteView):
    model = Pinch
    ...
    def get_success_url(self):
        ...
        return reverse("pinch:feed") + f"?author={self.author.slug}"

    def get_queryset(self):
        if is_moderator(self.request.user):
            return Pinch.objects.all()
        return Pinch.objects.filter(author=self.author)
...
def toggle_like(request, slug):
    _require_public_area_access(request)
    item = get_object_or_404(Pinch, slug=slug, status=Pinch.Status.APPROVED)
    ...
    if created:
        track_event(request, "content_like", object_type="pinch", object_id=item.pk, object_title=item.title)
    else:
        reaction.delete()
    if is_fetch:
...
def toggle_save(request, slug):
    ...
    if created:
        track_event(request, "collection_add", object_type="pinch", object_id=item.pk, object_title=item.title)
    else:
        saved.delete()
        track_event(request, "collection_remove", object_type="pinch", object_id=item.pk, object_title=item.title)
    if is_fetch:
...
def moderate(request, slug):
    ...
    if action == "approve":
        ...
        if not note:
            ...
            return redirect(item.get_absolute_url())
        item.status = Pinch.Status.NEEDS_CHANGES
        ...
        if not note:
            ...
            return redirect(item.get_absolute_url())
        item.status = Pinch.Status.REJECTED
        item.moderation_note = note
        item.moderated_by = request.user
        item.moderated_at = timezone.now()
        item.save(update_fields=["status", "moderation_note", "moderated_by", "moderated_at", "updated_at"])
        ...
        title = item.title
        item.status = Pinch.Status.ARCHIVED
        item.moderation_note = ""
        item.moderated_by = request.user
        item.moderated_at = timezone.now()
        item.save(update_fields=["status", "moderation_note", "moderated_by", "moderated_at", "updated_at"])
...
def generate_from_recipe(request, slug):
    """Create an Pinch pre-filled from an approved recipe.
    ...
    existing = Pinch.objects.filter(
        author=author,
        linked_recipe=recipe,
    ).exclude(status=Pinch.Status.ARCHIVED).first()

    if existing:
        messages.info(request, "An Pinch for this recipe already exists.")
    ...
    item = Pinch.objects.create(
        ...
        content_type=Pinch.ContentType.BEHIND_THE_DISH,
        linked_recipe=recipe,
    ...
    messages.info(
        request,
        "Legal details copied from the original recipe. Please review and confirm them for this Pinch.",
    )
    return redirect(
...
def generate_from_article(request, slug):
    """Create an Pinch pre-filled from an approved article.
    ...
    if existing:
        messages.info(request, "An Pinch for this article already exists.")
        return redirect(existing.get_absolute_url())

    _article_source_map = {
        "adapted": Pinch.SourceType.OTHER,
        "inspired": Pinch.SourceType.OTHER,
    }
    source_type = _article_source_map.get(article.source_type, Pinch.SourceType.ORIGINAL)
    ...
    messages.info(
        request,
        "Legal details copied from the original article. Please review and confirm them for this Pinch.",
    )
    return redirect(
        reverse("pinch:edit", kwargs={"slug": item.slug}) + "?from_article=1"
    )
    ...
    if item.allow_comments:
        comments = list(
            ...
            .prefetch_related(
                Prefetch(
                    "replies",
                    queryset=PinchComment.objects.filter(is_deleted=False)
                    .select_related("user", "user__recipe_author_profile"),
...
def submit_comment(request, slug):
    """Post a top-level comment or a reply on an Pinch item."""
    item = get_object_or_404(Pinch, slug=slug, status=Pinch.Status.APPROVED, allow_comments=True)
    if not item.is_announcement:
        _require_public_area_access(request)
    is_fetch = request.headers.get("X-AB-Fetch") == "1"
    if getattr(request, "limited", False):
