...
_OWN_WORK_LABEL = mark_safe(
    "This Pinch is my own original work or a properly credited adaptation."
)
...
class PinchAuthoringForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if (
            image_rights_status in {
                Pinch.ImageRightsStatus.LICENSED,
                Pinch.ImageRightsStatus.PUBLIC_DOMAIN,
            }
        ...
        if source_type and source_type not in {
            Pinch.SourceType.ORIGINAL,
            Pinch.SourceType.AI_ASSISTED,
        }:
