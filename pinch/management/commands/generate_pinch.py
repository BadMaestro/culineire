...
AI_SOURCE_NOTE = (
    "An original CulinEire Pinch, crafted with AI and reviewed by our editorial team."
)

CONTENT_TYPE_VALUES = [ct.value for ct in Pinch.ContentType]


def _map_content_type(value: str) -> str:
    raw = (value or "").strip().lower().replace(" ", "_").replace("-", "_")
    labels = {ct.label.lower().replace(" ", "_"): ct.value for ct in Pinch.ContentType}
    values = {ct.value: ct.value for ct in Pinch.ContentType}
    return values.get(raw) or labels.get(raw) or Pinch.ContentType.IRISH_BITE


def _prompt_for_pinch(topic: str, hint_type: str = "") -> str:
    types_list = ", ".join(CONTENT_TYPE_VALUES)
    type_hint = f' Use "{hint_type}" as the content_type.' if hint_type else ""
    return (
        f'Create an original CulinEire Pinch post about "{topic}". '
...
def _call_anthropic(topic: str, hint_type: str = "") -> dict:
    ...
    payload = {
        ...
        "max_tokens": 800,
        "messages": [{"role": "user", "content": _prompt_for_pinch(topic, hint_type)}],
    }
    ...
    for attempt in range(1, 4):
        ...
        try:
            ...
            if exc.code in (429, 500, 529) and attempt < 3:
                logger.warning("generate_pinch: Anthropic HTTP %d attempt %d — retrying in 10s", exc.code, attempt)
                time.sleep(10)
                continue
            ...
            last_exc = exc
            if attempt < 3:
                logger.warning("generate_pinch: Anthropic attempt %d failed (%s) — retrying in 5s", attempt, exc)
                time.sleep(5)
            else:
...
def _normalise_payload(payload: dict, topic: str, status: str) -> dict:
    ...
    if not title:
        raise CommandError("Generated Pinch has no title.")
...
class Command(BaseCommand):
    help = "Generate AI-assisted Pinch posts. Always saved as draft or pending — never auto-published."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--status",
            choices=[Pinch.Status.DRAFT, Pinch.Status.PENDING],
            default=Pinch.Status.PENDING,
        )
    ...
    def handle(self, *args, **options):
        ...
        for topic in topics:
            try:
                ...
                item = Pinch.objects.create(author=author, **fields)
                logger.info("generate_pinch: created %r (%s) #%d", item.title, item.status, item.pk)

                # Image generation — reuses recipes pipeline
                openai_key = getattr(settings, "OPENAI_API_KEY", "")
                if not openai_key:
                    logger.warning("generate_pinch: OPENAI_API_KEY not set — skipping image for %r", item.title)
                elif options.get("no_image"):
                    logger.info("generate_pinch: --no-image set — skipping image for %r", item.title)
                else:
                    try:
                        ...
                        logger.info("generate_pinch: cover image saved for %r", item.title)
                    except Exception as exc:
                        logger.error("generate_pinch: image failed for %r: %s", item.title, exc)
                ...
                logger.error("generate_pinch: failed for %r: %s", topic, exc)
                if len(topics) > 1:
                    self.stderr.write(self.style.ERROR(f'Failed "{topic}": {exc}'))
                    continue
                raise
            except Exception as exc:
                logger.error("generate_pinch: unexpected failure for %r: %s", topic, exc, exc_info=True)
                if len(topics) > 1:
                    self.stderr.write(self.style.ERROR(f'Failed "{topic}": {exc}'))
                    continue
                raise
