...
(function () {
  ...
  /* ── Battle Cancellation Simulation (MC01) ─────────────────────────────
   * A rehearsal, not an action. It talks to no endpoint and posts nothing: the
   * whole flow is already in the page, one <section> per step, and this only
   * decides which one is on screen.
   *
   * The flow branches twice, so the stepper carries the set of paths still
   * POSSIBLE rather than a step index. A step is shown when it belongs to every
   * path still possible; at a fork nothing qualifies, which is exactly why Next
   * goes dead there until a branch is picked. That falls out of the rule instead
   * of being a second condition someone has to keep in step with the first.
   */
  (function simCancellation() {
    var panel = document.getElementById('amc-sim-cancellation');
    if (!panel) return;

    var stage = document.getElementById('amc-sim-stage');
    var track = document.getElementById('amc-sim-track');
    var runBtn = document.getElementById('amc-sim-run');
    var resetBtn = document.getElementById('amc-sim-reset');
    var backBtn = document.getElementById('amc-sim-back');
    var nextBtn = document.getElementById('amc-sim-next');
    var pathOut = document.getElementById('amc-sim-path');
    var steps = [].slice.call(panel.querySelectorAll('[data-sim-step]'));
    if (!stage || !steps.length) return;

    var ALL = ['waived', 'upheld', 'overturned'];
    var PATH_TEXT = {
      waived: 'The other chef waived the penalty.',
      upheld: 'The penalty was asked for and the moderator upheld it.',
      overturned: 'The penalty was asked for and the moderator refused it.'
    };

    var candidates = ALL.slice();
    var cursor = 0;
    var history = [];

    function pathsOf(el) {
      return (el.getAttribute('data-sim-paths') || '').split(/\s+/).filter(Boolean);
    }

    function visible() {
      return steps.filter(function (el) {
        var p = pathsOf(el);
        return candidates.every(function (c) { return p.indexOf(c) !== -1; });
      });
    }
    ...
    function render() {
      ...
      if (shown[cursor]) shown[cursor].hidden = false;
      ...
      shown.forEach(function (el, i) {
        ...
        li.setAttribute('data-state',
          i === cursor ? 'current' : (i < cursor ? 'done' : 'ahead'));
        track.appendChild(li);
      });

      backBtn.disabled = cursor === 0 && !history.length;
      nextBtn.disabled = cursor >= shown.length - 1;
      pathOut.textContent = candidates.length === 1
        ? PATH_TEXT[candidates[0]]
        : 'Step ' + (cursor + 1) + ' of ' + shown.length +
          (nextBtn.disabled ? ' - choose what happens next' : '');
    }

    function choose(path) {
      history.push({ candidates: candidates.slice(), cursor: cursor });
      /* "Ask for the penalty" leaves TWO paths open, because the moderator has
       * not ruled yet. Narrowing to the intersection keeps that honest. */
      var picked = (path === 'upheld' && candidates.length === 3)
        ? ['upheld', 'overturned']
        : [path];
      candidates = picked.filter(function (c) { return candidates.indexOf(c) !== -1; });
      cursor += 1;
      render();
    }

    panel.addEventListener('click', function (ev) {
      var btn = ev.target.closest ? ev.target.closest('[data-sim-path]') : null;
      if (btn) choose(btn.getAttribute('data-sim-path'));
    });
    ...
    nextBtn.addEventListener('click', function () { cursor += 1; render(); });

    backBtn.addEventListener('click', function () {
      if (cursor > 0) { cursor -= 1; render(); return; }
      var prev = history.pop();
      if (prev) { candidates = prev.candidates; cursor = prev.cursor; render(); }
    });
  })();
