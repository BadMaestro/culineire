...
class ArenaMasterConsoleAccessTests(TestCase):
    ...
    def test_all_eight_panels_present_with_disabled_controls(self):
        ...
        import re
        # OPERATOR controls, not every button on the page. The scenario panel
        # (MC01) has its own buttons in the same visual style, and they are not
        # operator actions - they move a rehearsal forward and touch nothing.
        # Counting them here made this assertion a page-wide button census that
        # every new panel breaks; data-amc-sim marks them out.
        amc_buttons = [
            b for b in re.findall(r'<button[^>]*class="amc-btn[^"]*"[^>]*>', content)
            if "data-amc-sim" not in b
        ]
...
class ArenaSpectatorTests(TestCase):
    """Stage 3C: interactive spectator seats are real ArenaSeat holders only."""
    ...
    def test_online_viewers_only(self):
        ...
        viewer = self._author("viewer", enrolled=False, seen_secs_ago=10)  # in
        claim_seat(viewer)
        claim_seat(self._author("chef", enrolled=True, seen_secs_ago=10))  # out: enrolled filter
        stale = self._author("stale", enrolled=False, seen_secs_ago=9999)
        claim_seat(stale)  # claimed while "online" then went stale — lapses
        self._author("never", enrolled=False, seen_secs_ago=None)
        suspended = self._author("suspended", enrolled=False, suspended=True)
        claim_seat(suspended)
        cutoff = timezone.now() - timezone.timedelta(seconds=180)
        specs = _get_spectators(cutoff, viewer_author=viewer)
    ...
    def test_unseated_online_viewer_is_not_poured_into_a_cell(self):
        """Presence alone must not invent a seat — only ArenaSeat does."""
        from chef_battle.views import _get_spectators
        self._author("loose", enrolled=False, seen_secs_ago=5)
        cutoff = timezone.now() - timezone.timedelta(seconds=180)
        self.assertEqual(_get_spectators(cutoff), [])
    ...
    def test_get_spectators_respects_capacity_limit(self):
        ...
        for i in range(3):
            author = RecipeAuthor.objects.create(
                user=get_user_model().objects.create_user(f"cap-{i}", password="pw"),
                name=f"Cap {i}", slug=f"cap-{i}",
            )
...
class BattleCancellationSimulationTests(TestCase):
    """MC01 - the withdrawal, walked step by step inside the Master Console.

    The Owner, 2026-08-05: the flow shipped in v2.5.830 "тоже должен быть частью
    Master Console Panel Battle Cancellation Simulation - и при нажатии на этот
    сценарий - показывать всё по шагам как это будет выглядеть на реальной
    арене."

    Two things have to stay true, and both are load-bearing:

      * it WRITES NOTHING, so the operator can run it against the live battle on
        production as often as he likes;
      * every figure in it is read off the real model and the real service, so a
        change to the rule cannot leave the rehearsal teaching something untrue.
    """

    def setUp(self):
        User = get_user_model()
        user = User.objects.create_superuser("mc01-operator", password="pw")
        RecipeAuthor.objects.create(
            user=user, name="MC01 Operator", slug="mc01-operator",
            has_arena_console_access=True,
        )
        self.client.login(username="mc01-operator", password="pw")

    def test_the_console_carries_the_scenario(self):
        resp = self.client.get(reverse("chef_battle:master_console"))
        self.assertEqual(resp.status_code, 200)
        self.assertContains(resp, "Battle Cancellation Simulation")
        self.assertContains(resp, 'id="amc-sim-cancellation"')
        self.assertIn("cancellation_simulation", resp.context)

    def test_every_step_is_rendered_with_its_paths(self):
        """The stepper picks a step by the paths it belongs to. A step without
        them is invisible for every route, which is a silent hole."""
        resp = self.client.get(reverse("chef_battle:master_console"))
        sim = resp.context["cancellation_simulation"]
        self.assertGreaterEqual(len(sim["steps"]), 6)
        for step in sim["steps"]:
            self.assertTrue(step["paths"], f"{step['key']} belongs to no path")
            self.assertTrue(step["screen"], f"{step['key']} shows nothing")
            self.assertTrue(step["rules"], f"{step['key']} enforces nothing")
            self.assertTrue(step["effects"], f"{step['key']} changes nothing")
            self.assertContains(resp, f'data-sim-step="{step["key"]}"')

    def test_the_three_routes_each_reach_exactly_one_ending(self):
        """The visibility rule is: a step shows when it belongs to every path
        still possible. Walk it here so a reordered spine cannot leave a route
        with two endings, or none."""
        from .console_simulations import PATH_OVERTURNED, PATH_UPHELD, PATH_WAIVED

        sim = build_cancellation_simulation()
        endings = {"closed_clean", "closed_penalty"}
        for path in (PATH_WAIVED, PATH_UPHELD, PATH_OVERTURNED):
            reached = [s["key"] for s in sim["steps"] if path in s["paths"]]
            self.assertEqual(
                len(endings.intersection(reached)), 1,
                f"path {path} reaches {endings.intersection(reached)}",
            )
            self.assertEqual(reached[0], "entry")

    def test_the_figures_come_from_the_rule_and_not_from_prose(self):
        from .models import BattleWithdrawal
        from .withdrawal_service import WITHDRAWAL_ALLOWANCE

        sim = build_cancellation_simulation()
        self.assertEqual(sim["allowance"], WITHDRAWAL_ALLOWANCE)
        self.assertEqual(sim["penalty_rating"], BattleWithdrawal.PENALTY_RATING)
        self.assertEqual(sim["penalty_reputation"], BattleWithdrawal.PENALTY_REPUTATION)

        penalty_step = next(s for s in sim["steps"] if s["key"] == "closed_penalty")
        blob = " ".join(penalty_step["effects"])
        self.assertIn(str(BattleWithdrawal.PENALTY_RATING), blob)
        self.assertIn(str(BattleWithdrawal.PENALTY_REPUTATION), blob)

    def test_the_no_penalty_ending_takes_nothing(self):
        """A withdrawal is a cancellation, not a defeat. If the clean ending ever
        starts describing a loss, the rehearsal has stopped matching the code."""
        sim = build_cancellation_simulation()
        clean = next(s for s in sim["steps"] if s["key"] == "closed_clean")
        blob = " ".join(clean["effects"]).lower()
        self.assertIn("cancelled", blob)
        self.assertIn("untouched", blob)

    def test_running_the_scenario_writes_nothing(self):
        """The whole safety case. Every row count is taken before and after a
        load of the console, and the simulation is built in that load."""
        from .models import Battle, BattleWithdrawal

        before = (
            Battle.objects.count(),
            BattleWithdrawal.objects.count(),
            ChefBattleProfile.objects.count(),
            BattleEvent.objects.count(),
        )
        resp = self.client.get(reverse("chef_battle:master_console"))
        self.assertEqual(resp.status_code, 200)
        after = (
        ...
        self.assertEqual(before, after, "the simulation wrote to the database")

    def test_an_empty_arena_says_so_instead_of_inventing_chefs(self):
        sim = build_cancellation_simulation(None)
        self.assertFalse(sim["cast"]["live"])
        self.assertIsNone(sim["cast"]["battle_id"])
        self.assertIn("stand-in", sim["cast"]["theme"])


class SimulationMatchesTheRealFlowTests(TestCase):
    """The rehearsal is checked against the thing it rehearses.

    MC01 forbids re-implementing the withdrawal rule, and a walkthrough written
    in prose is exactly the shape that drifts: the code changes, the description
    does not, and the operator is taught something that stopped being true. The
    constants are imported rather than typed, which covers the numbers; this
    covers the SENTENCES, by running the real service down each path and
    checking that what the simulation promised is what happened.
    """

    def setUp(self):
        User = get_user_model()
        self.user_a = User.objects.create_user(username="sim-a", password="pw")
        self.user_b = User.objects.create_user(username="sim-b", password="pw")
        self.mod = User.objects.create_user(username="sim-mod", password="pw", is_staff=True)
        self.chef_a = RecipeAuthor.objects.create(user=self.user_a, name="Leaver", slug="sim-a")
        self.chef_b = RecipeAuthor.objects.create(user=self.user_b, name="Stayer", slug="sim-b")
        now = timezone.now()
        self.battle = Battle.objects.create(
            challenger=self.chef_a, opponent=self.chef_b, theme="Simulated theme",
            status=Battle.Status.SCHEDULED, start_time=now,
            submission_deadline=now + timezone.timedelta(hours=12),
            end_time=now + timezone.timedelta(days=1),
        )

    def _walk(self, *, with_penalty, uphold):
        from .services import get_or_create_battle_profile
        from .withdrawal_service import (
            decide_withdrawal, request_withdrawal, resolve_withdrawal,
        )

        before = get_or_create_battle_profile(self.chef_a)
        start = (before.rating, before.reputation, before.losses,
                 before.withdrawals_remaining)

        w = request_withdrawal(battle=self.battle, author=self.chef_a, reason="Kitchen fire.")
        spent = get_or_create_battle_profile(self.chef_a).withdrawals_remaining
        decide_withdrawal(withdrawal=w, author=self.chef_b, with_penalty=with_penalty,
                          opponent_reason="No warning at all." if with_penalty else "")
        resolve_withdrawal(withdrawal=w, moderator=self.mod, uphold_penalty=uphold)

        self.battle.refresh_from_db()
        after = get_or_create_battle_profile(self.chef_a)
        return start, spent, after, w

    def test_the_allowance_is_spent_on_the_asking_as_the_simulation_says(self):
        sim = build_cancellation_simulation()
        reason_step = next(s for s in sim["steps"] if s["key"] == "reason")
        self.assertIn("ASKING", " ".join(reason_step["rules"]).upper())

        start, spent, _after, _w = self._walk(with_penalty=False, uphold=False)
        self.assertEqual(spent, start[3] - 1, "the allowance was not spent on the request")

    def test_the_clean_ending_takes_nothing_and_cancels_the_battle(self):
        start, _spent, after, w = self._walk(with_penalty=False, uphold=False)
        self.assertEqual(self.battle.status, Battle.Status.CANCELLED)
        self.assertEqual(after.rating, start[0])
        self.assertEqual(after.reputation, start[1])
        self.assertEqual(after.losses, start[2], "a withdrawal is not a defeat")
        self.assertFalse(w.penalty_applied)

    def test_the_moderator_can_refuse_a_penalty_the_other_chef_asked_for(self):
        """The third path exists in the simulation, so it has to exist in the code."""
        start, _spent, after, w = self._walk(with_penalty=True, uphold=False)
        self.assertEqual(after.rating, start[0])
        self.assertEqual(after.reputation, start[1])
        self.assertFalse(w.penalty_applied)
        self.assertEqual(self.battle.status, Battle.Status.CANCELLED)

    def test_the_penalty_ending_takes_exactly_what_the_simulation_promised(self):
        """A chef with rating to lose loses exactly the Owner's figure."""
        from .models import BattleWithdrawal
        from .services import get_or_create_battle_profile
        ...
        start, _spent, after, w = self._walk(with_penalty=True, uphold=True)
        self.assertEqual(after.rating, start[0] - BattleWithdrawal.PENALTY_RATING)
        self.assertEqual(after.reputation, start[1] - BattleWithdrawal.PENALTY_REPUTATION)
        self.assertEqual(after.losses, start[2], "no loss is recorded on a withdrawal")
        self.assertTrue(w.penalty_applied)
        self.assertEqual(self.battle.status, Battle.Status.CANCELLED)

    def test_the_rating_never_goes_below_zero_and_the_panel_says_so(self):
        """This is the one the rehearsal got wrong first.

        The panel used to promise a flat -15. penalise() floors the rating at
        zero, so a chef sitting on 0 - which is every new account - loses
        nothing at all, and the operator would have been shown a number that
        never happens. The panel now says so, and this keeps both honest.
        """
        sim = build_cancellation_simulation()
        penalty_step = next(s for s in sim["steps"] if s["key"] == "closed_penalty")
        self.assertIn("never below zero", " ".join(penalty_step["effects"]))
