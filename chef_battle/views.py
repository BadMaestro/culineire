...
def _build_battlefield_progress():
    ...
    phases = [
        ...
        },
        {
            ...
            "items": [
                {"label": "Fix appreciation gift catalogue", "detail": "Replace old gifts with: Coffee 20T, Virtual Beer Toast 30T, Virtual Whiskey Toast 50T, Flowers 80T, Celebration Cocktail 80T, Virtual Champagne Bottle 100T. Update DB, model choices and all templates.", "status": "pending"},
                {"label": "Token shop VAT button wording", "detail": "Change 'Buy €X.XX' buttons to 'Buy €X.XX + VAT' or 'Continue to checkout'. Net price must never appear as final consumer price. Update /chef-battle/tokens/ and /legal/purchases-and-vat/.", "status": "pending"},
                {"label": "Remove 'Battle Gifts are permanent'", "detail": "Replace with: 'Battle Gift artifacts are one-use digital combat items. If used in combat, the artifact is consumed and cannot be used again. A historical record may remain on the Chef profile or battle log.'", "status": "pending"},
                {"label": "Add 18+ rule to public rules", "detail": "Add visible 18+ section to /chef-battle/rules/ covering: battle access, token purchase, paid gifts, artifacts, CBR/LSR, Stripe Connect onboarding, payout, live video. Add author/chef upgrade path.", "status": "pending"},
                {"label": "Add closed-loop token model to rules", "detail": "Add public section: tokens are closed-loop digital credits, no cash value, not withdrawable, not transferable, not e-money, not CBR/LSR. Update /chef-battle/rules/ and /legal/purchases-and-vat/.", "status": "pending"},
                {"label": "Add anti-gambling wording", "detail": "Add public section: Chef Battle is not gambling, betting, staking, jackpot or prize-pool gameplay. Tokens may not be staked. Battle outcome does not transfer tokens between users. CBR/LSR only through approved platform logic.", "status": "pending"},
                {"label": "Add digital-only gift disclaimers", "detail": "Add wording near gifts: 'All gifts are digital appreciation items only. No physical alcohol, food, drink, bottle or goods are supplied.' Add alcohol-themed gift notice per §6 addendum.", "status": "pending"},
                {"label": "Add CBR/LSR section to rules", "detail": "Explain: CBR = Chef Battle Rewards, LSR = Live Support Rewards. Both are internal reward records, not tokens, not money until paid out. 10 reward statuses. Winning a battle does not automatically create a payout.", "status": "pending"},
                {"label": "Add Next Battle Unlock rule", "detail": "CBR/LSR from Battle N are not payout-eligible until Chef completes next eligible Battle N+1. Add to /chef-battle/rules/ with full eligible-battle definition.", "status": "pending"},
                {"label": "Add Stripe Connect payout section to rules", "detail": "Explain payout path: approved CBR/LSR only, after all unlock/verification/fraud/compliance checks. Rate: 1 reward token = €0.025. Min: €50 / 2000 tokens. Not automatic, not guaranteed.", "status": "pending"},
                {"label": "Add DAC7 / tax responsibility wording", "detail": "Add public wording: Chefs responsible for own tax affairs. CBR/LSR payouts may be taxable income. CulinEire does not give tax advice. May report to Revenue where legally required.", "status": "pending"},
                {"label": "Add refund / chargeback policy wording", "detail": "Add to /legal/purchases-and-vat/: tokens non-refundable once credited and used, except as required by law or confirmed technical/billing error. Chargeback consequences for reward records.", "status": "pending"},
                {"label": "Add account transfer ban to rules", "detail": "Accounts are personal and non-transferable. No selling, renting, sharing accounts or allowing another person to use them for Chef Battle, rewards, artifacts or payouts.", "status": "pending"},
                {"label": "Stripe token purchase webhook", "detail": "Wire Stripe live key and webhook for /chef-battle/stripe/webhook/. Events: checkout.session.completed and checkout.session.expired. Update STRIPE_WEBHOOK_SECRET in .env.", "status": "pending"},
                {"label": "Store extended payment data on token orders", "detail": "TokenOrder model must store: Stripe session ID, payment intent ID, customer ID, net amount, VAT amount, gross amount, currency, Stripe tax ref, invoice ref, payment/refund/dispute/token status, credited_at.", "status": "pending"},
            ],
        },
        {
            "title": "Phase 8 - Economy Protection (CBR / LSR / Ledger)",
            "items": [
                {"label": "RewardRecord model (CBR and LSR)", "detail": "New model: user/chef, reward_type (CBR/LSR), source_type, source_id, amount_tokens, status (10 statuses), created_at, unlock_required, unlock_battle_id, reviewed_by, reviewed_at, reversal_reason, payout_batch_id.", "status": "pending"},
                {"label": "LSR creation on eligible appreciation gift", "detail": "When a viewer sends an eligible appreciation gift, CulinEire creates a separate pending LSR record for the Chef. 1 Spendable Token spent = 1 pending LSR. No direct token transfer to Chef.", "status": "pending"},
                {"label": "Gift reward-eligibility flag", "detail": "AppreciationGiftType must have reward_eligible flag and reward_basis field. Artifact cost must never create LSR. Only configured support component of Battle Gift can create LSR.", "status": "pending"},
                {"label": "Artifact consumption tracking", "detail": "ChefArtifactInstance model with status: available / reserved / consumed / expired / reversed. When artifact is used in battle it is consumed immediately and cannot be reused. Combat log entry preserved.", "status": "pending"},
                {"label": "Admin artifact grant with audit", "detail": "Staff-only UI to grant artifact to Chef with mandatory reason field. Admin grant creates ChefArtifactInstance with source=admin_grant and immutable audit log entry. Never creates CBR/LSR.", "status": "pending"},
                {"label": "Next Battle Unlock logic", "detail": "CBR/LSR from Battle N locked until Chef completes eligible Battle N+1. Eligibility check: challenge accepted, recipe submitted, combat done, cooking done, dish photo uploaded, moderator approved, voting done, not abandoned/disqualified/cancelled/fraud-flagged.", "status": "pending"},
        ...
        },
        {
            ...
            "items": [
                {"label": "PayoutRequest model", "detail": "New model: chef, amount_reward_tokens, payout_rate_snapshot, gross_payout_amount, currency, Stripe Connect account ID, status, requested_at, reviewed_by, reviewed_at, paid_at, Stripe payout reference, rejection_reason, compliance_flags.", "status": "pending"},
                {"label": "Stripe Connect onboarding flow", "detail": "Chef onboarding: age 18+, approved Chef status, Chef Reward Agreement accepted, Stripe Connect account creation, identity/payout verification, tax/reporting data collection (legal name, DOB, address, country of tax residence, tax ID, business details).", "status": "pending"},
                {"label": "Chef Reward Agreement", "detail": "Chef must accept Chef Reward Agreement before becoming payout-eligible. Agreement covers: CBR/LSR rules, payout conditions, tax responsibility, DAC7/MRDP obligations, account transfer ban.", "status": "pending"},
                {"label": "DAC7 / MRDP data collection", "detail": "Collect and store: legal name, DOB, primary address, country of tax residence, tax identification number, business/entity details, Stripe Connect reference, total consideration paid, fees/deductions, reporting period, seller verification status, reportable status, timestamps.", "status": "pending"},
                {"label": "Payout request flow", "detail": "Chef can request payout only when: 18+, approved Chef, Stripe Connect onboarded, Chef Reward Agreement accepted, Next Battle Unlock complete, minimum 2000 approved reward tokens / €50.00, no active fraud/compliance/chargeback flags, admin review pending.", "status": "pending"},
                {"label": "Admin payout approval", "detail": "Staff review panel for payout requests: approve, reject, delay. Rate snapshot locked at request time: 1 approved reward token = €0.025. Approved payout triggers Stripe Connect transfer.", "status": "pending"},
                {"label": "Payout ledger and statements", "detail": "All payout events recorded in immutable ledger. Chefs can view their payout history, reward token balance, and pending/locked/approved reward records.", "status": "pending"},
            ],
        },
        {
            "title": "Phase 10 - Live Video Round 2",
            "items": [
                {"label": "LiveBroadcast model", "detail": "New model: battle, chef, stream_status, started_at, ended_at, recording_reference, moderation_status, safety_delay_enabled, stopped_by_staff, stop_reason, report_count, reviewed_by, reviewed_at. Plus LiveBroadcastReport model.", "status": "pending"},
                {"label": "Live stream infrastructure", "detail": "Choose streaming provider (WebRTC / HLS / third-party). Implement 30-60 second broadcast delay where possible. Only verified 18+ approved Chefs can stream.", "status": "pending"},
                {"label": "Pre-live checklist", "detail": "Chef must confirm 14-point checklist before going live: age 18+, approved Chef, minors out of frame, cooking area only, no personal documents visible, no copyrighted content, safe kitchen, understand injury rules, stream may be recorded, CulinEire may stop stream.", "status": "pending"},
                {"label": "Live Battle Agreement", "detail": "First-time live stream requires Chef to accept Live Battle Agreement covering all live video rules (minors, safety, prohibited content, alcohol, brands, privacy, copyright, defamation).", "status": "pending"},
                {"label": "Staff / admin emergency stop", "detail": "Staff can immediately stop any live stream. Stop event recorded in LiveBroadcast model with reason and staff ID. Payout/reward eligibility can be locked if stream breach affects battle integrity.", "status": "pending"},
                {"label": "Live stream report button", "detail": "Viewers can report a live stream. Report creates LiveBroadcastReport record. Auto-hide or auto-pause trigger on high-volume rapid reports. Report categories: child safety, privacy breach, prohibited content, alcohol/drug misuse, illegal content, copyright breach.", "status": "pending"},
                {"label": "Recording and moderation review", "detail": "Streams recorded for moderation. Post-stream review before recording is made public. Staff can: remove, blur, edit, hide or refuse to publish any recording.", "status": "pending"},
                {"label": "Live video rules in public rules page", "detail": "Add sections to /chef-battle/rules/: Live Video Round 2, Minors and Home Kitchens, Kitchen Safety, Prohibited Content, Alcohol, Brands, Privacy, Music and Copyright, Defamation and Food Safety, Pre-Live Checklist.", "status": "pending"},
            ],
        },
        {
