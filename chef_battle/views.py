    ...
    phases = [
        {
            ...
            "items": [
                ...
                {"label": "Feature flag in place", "detail": "CHEF_BATTLE_ENABLED controls homepage queries and battle URLs. It is the ONE-WAY LAUNCH LATCH - False until launch, True after it, never back (Owner; F65 is reviewed, not a bug). This row is derived from the live setting, and its old text claimed the flag was already enabled on production while the derived status said pending: prose contradicting the value beside it. Turning it on IS the launch, so this row goes done at launch and not before.", "status": "done" if feature_enabled else "pending", "completed_at": "2026-06-10"},
                {"label": "Sandbox enablement confirmed", "detail": "The same latch, read from the server rather than from a sentence. Its old text asserted that CHEF_BATTLE_ENABLED=True had already been applied on production, which was never true and sat next to a derived status saying pending. Confirmation is the flag itself: this row turns done the moment the setting does.", "status": "done" if feature_enabled else "pending", "completed_at": "2026-06-10"},
        ...
        },
        {
            ...
            "items": [
                ...
                {"label": "TikTok / Instagram live integration", "detail": "SEASON 2, by the Owner's ruling of 2026-08-17 - card T26, deferred and unassigned the same day the access TOR was sent to him. Confirmed with him that he wants a REAL live stream (arena/camera feed pushed into TikTok/Instagram Live), not a social-media link - those already exist in the site footer. Before any code can exist: a TikTok developer account plus a Live Streaming API application, and a Meta Business Account plus business verification plus Instagram Graph API App Review. Both are external, identity-verified processes only he can start, each with a 1-3 week turnaround, and nothing in this repository talks to either platform. Re-open when at least one platform Live API is approved.", "status": "pending"},
            ],
        },
        {
