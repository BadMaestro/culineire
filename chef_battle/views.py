    ...
    phases = [
        ...
        },
        {
            ...
            "items": [
                ...
                {"label": "Surface real Crown holder at arena centre", "detail": "battle_crown_holder (real ChefBattleProfile, crown_until > now) is not yet in the arena_data JSON consumed by the SVG centre renderer. Needs a new center.type == 'crown' branch in arena()/arena_state() (chef_battle/views.py) plus a drawCentre() branch in arena_puzzle.js reusing the existing crown visual language.", "status": "pending"},
                {"label": "Wire blast-ring to a real site-wide win event", "detail": "BattleEvent(event_type=BATTLE_COMPLETED / CROWN_AWARDED) is real, already-logged data with usable message text and battle.result_reason as a ready-made score string. .battle-blast has zero JS trigger anywhere. Needs a lightweight poll (matching the existing pingArena/pollArenaState/notifications_poll pattern already used site-wide -- no WebSocket/Channels infrastructure exists in this project) so ANY arena visitor sees the celebration when ANY battle concludes, not just participants.", "status": "pending"},
                ...
                {"label": "Curate EPIC/LEGENDARY artifact names", "detail": "chef_battle/fixtures/battle_artifacts.json: real counts are 21 EPIC + 11 LEGENDARY (32 total, not 24+16 as originally briefed). Several already match the Irish-myth/culinary-imagery target pattern (Cauldron of Lugh, The Irish Kitchen, Dagda's Cauldron, The Eternal Apron, Manannan's Cloak). Roughly a dozen still carry generic Western-fantasy/Greek/Norse naming (Dragon Wok, Dragon Scale Shield, Aegis Stockpot, Excalibur Cleaver, Poseidon's Trident, The CulinEire Sword, Grail Chalice, Olympian Bain Marie, Phoenix Casserole, Viking Stockpot, Titan Roaster, Mithril Oven Gloves, Prometheus Flame) and need rewriting. COMMON/UNCOMMON/RARE tiers are untouched by design.", "status": "pending"},
