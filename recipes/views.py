...
ARENA_DESIGN_TASKS = [
    ...
    {
        "id": "T23", "group": "Chef Battles roadmap (Owner list, 2026-08-16)", "title": "Clan and team battles - SEASON 2",
        ...
        "evidence": "DEFERRED TO SEASON 2 by the Owner, 2026-08-17 - and it turns out he had already ruled this on 2026-07-14, in writing, before the card existed. docs/chef_battle/clans_alliances_rules.md section 2 splits the mechanic in two: 'Season 1 (now): minimal foundation only - the alliance entity, the clan-alliance link, and the call-an-ally-into-a-battle hook. Season 2 (later): the full cuisine-vs-cuisine popularity mechanic.' The same section states what a team fight even IS in this game: with allies involved the fight becomes a battle OF A CUISINE, not of a single recipe - a dispute about which cuisine is more popular this season. That is a different product from the 1v1 recipe duel, not an extension of it. WHY IT COULD NEVER HAVE BEEN A SMALL CHANGE. Battle is hard-wired to two chefs: challenger and opponent as FKs, plus a database CHECK constraint forbidding them to be equal, and everything downstream assumes exactly two - combat actions per chef, opponent_for(chef), declared ingredients per side, voting, scoring. A team format changes the shape of the model rather than adding to it. SEASON-1 SCOPE, MEASURED: the alliance entity exists (Alliance), the clan-alliance link exists (AllianceMembership, one active alliance per clan, enforced by a partial unique constraint), and THE CALL-AN-ALLY HOOK DOES NOT EXIST - repo-wide search finds nothing. That hook is the only part of this in scope before Season 2, and it is not yet assigned. ALSO MEASURED, ON PRODUCTION, because it changes how urgent any of this is: zero clans, zero alliances, zero clan memberships, four battles. The clan mechanic has never been used by anybody. Put to the Owner as an open question rather than assumed either way.",
    },
    {
    ...
    {
        "id": "T26", "group": "Chef Battles roadmap (Owner list, 2026-08-16)", "title": "TikTok / Instagram live integration",
        "status": "ASSIGNED", "owner": "Bolt",
        "files": "unbuilt - no existing code in this repo integrates either platform",
        "depends_on": "none",
        "action": "The battlefield board's own line (views.py:226) already states the blocker: 'Stream cooking phase live. Requires platform account verification and API approval.' Nothing in this repository talks to either platform's API. Before any code: platform developer accounts, API access approval, and a decision on what 'cooking phase live' actually streams (screen, camera feed, existing photo uploads) are all his to obtain or decide, not an agent's.",
        "visible_result": "Not yet defined - blocked on external accounts this project does not hold.",
        ...
        "evidence": "CREATED 2026-08-16 on the Owner's instruction (list item 5), assigned to Bolt. BLOCKED on external platform access he has not yet obtained.",
    },
    {
...
ARENA_RELEASE_STAGES = [
    ...
    },
    {
        ...
        "commit": "66867086 / production v2.5.1123",
