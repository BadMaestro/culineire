...
ARENA_RELEASE_STAGES = [
    ...
    },
    {
        ...
        "owners": "3A/3B/3C/3E focused-done; 3D open (D3); 3G in progress (R5 needs D2). Pool: Cursor carrying; Bolt/Ember weekly-limited.",
        "criteria": ["All mandatory workstreams complete.",
                     ...
                     "3A/3B/3C deployed; 3E focused verify DONE (8:6:1 not claimed); 3D remains open pending Owner D3.",
        ...
        "verification": "3A distributed verified. 3B authenticated re-QA 19/19. 3C DEPLOYED v2.5.559. 3E FOCUSED VERIFY DONE v2.5.561 (post-R4; 8:6:1 not claimed). CF5 DEPLOYED v2.5.558. 3D remains OPEN (Owner D3). R4 DEPLOYED v2.5.560.",
        "updated": "2026-07-24T18:55:00.000Z",
        "next_action": "Owner D3 country/flag (closes 3D). Owner D2 stands/atmosphere (unblocks R5). AF atmosphere STOPPED.",
        "workstreams": [
            ...
            {"id": "3C", "title": "Real spectator overlay", "owner": "ArenaFront (FE) + Cursor (BE) — 3C pair",
             "status": "DEPLOYED — oval stands (cap 290); mechanisms preserved; was 544 polar",
             "requirements": ["Atmospheric crowd presentation may match the approved mockup but must not impersonate registered or online users.",
                              "Keep interactive spectator seats for real viewers only (Owner 2026-07-24 oval stands, capacity 290), assigned front/near rows first; do not place synthetic occupants in interactive seats.",
                              "A logged-in visitor can see themselves seated."],
             "evidence": "3C real-viewer law kept. MOCKUP_OCTAGON_OVAL DEPLOYED: polar 544 rings replaced by spectator oval (L3/R3/T2/B2) capacity 290 — Owner geometry change 2026-07-24; mechanisms (claim/appear/move/poll) preserved. FE tip ff141240. Product contract §12 544 still needs Owner amend (honesty recorded). AF atmosphere STOPPED."},
            {"id": "3D", "title": "Chef confrontation panels", "owner": "ArenaFront (FE) + Cursor (BE) — 3R3 pair",
             "status": "PARTIAL — 3R3 DEPLOYED (flag OUT until D3)",
             ...
             "evidence": "3R3 DEPLOYED as v2.5.551 from tip e22acd33 (agent/arenafront/3r3-confrontation). Compositional half CLOSED: SSR+poll bind name/avatar_url/slug/profile_url/side via _arena_fighter_payload + arena_deck.js/arena.html. Flag/country still OUT (Owner D3). 3D remains OPEN until D3. AF atmosphere STOPPED."},
            {"id": "3E", "title": "Rank floor column", "owner": "GreenBear + Cursor (Ember's suspended VERIFY_AND_CLOSE_STAGE_3E is now ours)",
             "status": "DONE — FOCUSED VERIFY (R4 Option B); 8:6:1 not claimed",
             ...
             "evidence": "FOCUSED VERIFY CLOSED 2026-07-24 by Cursor on prod 79c4d2b1 / R4 v2.5.560. ArenaRankColumnTests 10/10 PASS (8 model ranks, ol markup, aria-labels, mobile not display:none, contrast >=7:1 from --ink/--surface, D1 Option B centred stack guard). Audit refreshed: ops/audits/stage_3e_rank_column.json verification_close_2026_07_24. Contrast still 14.30:1 / 12.62:1. Distributed 8:6:1 NOT RUN (Owner D4 exemption honesty). Authenticated browser QA NOT CLAIMED."},
            ...
            {"id": "3G", "title": "Visual integration (composition redesign)", "owner": "Cursor (composition/polish); ArenaFront (atmosphere assets)",
             "status": "IN PROGRESS — the live Arena does not look like the mockup. Owner compared them 2026-07-27 and the verdict was: nothing to show.",
             ...
             "evidence": "R0/R1/R3/R4/R6/CF5/3C done. MOCKUP_OCTAGON_OVAL: chef rings 9..40 + spectator oval cap 290 (FE ff141240). R4 v2.5.560. Capacity honesty 544→290 (Owner geometry). R5/D2 still open. D3 open. GAP TO THE MOCKUP, ranked by Bolt from the Owner's own screenshot on 2026-07-27, worst first: (1) THE HALL IS NOT DARK — in the mockup the light parchment floor works because it is the only bright thing in a near-black hall; live, everything is pale, so nothing reads, including the faces that were wired the night before. This is the root cause and every other item is cosmetic until it is fixed. (2) Visual weight inverted: floor fills ~80% of the frame while the crowd is squeezed into thin rings at the edge; in the mockup weight comes from density and value, not radius. (3) Faces clamped to the seat marker by Math.min(faceDiameter, seatSize) in arena_render.js, so a portrait can never be larger than its grey dot; in the mockup heads are large and overlap slightly and no seat marker is visible under them. (4) No depth falloff across the rows. (5) No gold rim light at the floor/stands boundary. (6) No slate band between floor and stands. (7) No chef confrontation panels left and right. Step 1 (dark hall) issued to Cursor and ArenaFront as CoWork #3289/#3290 on 2026-07-27; acceptance is the OWNER'S SCREENSHOT, not a green test, because Bolt has no sight of the Arena at all.",
            ...
            {"id": "3H", "title": "Geometry: proportions and camera (G1..G5)", "owner": "Cursor (implementation); Bolt (Production Director, acceptance)",
             "status": "DONE — all six gates measured in a browser at three widths. G5 @ 1187a8c3.",
             ...
             "evidence": "G1 impl/arena-g1-proportions @ a81af083: floor_outer 296.25, stands_outer 474.0, ratio 1.5999, stage 0.13, FLOOR_SHARE 0.63. Verified by Bolt against the diff rather than the report: selectors.py diff empty, no rotateX added, 3 files. G2 impl/arena-g2-camera @ e181e1a4, stacked on G1 and not on main: rotateX(56deg), perspective 1500px, overflow switched to visible so the stands stop clipping, and all four stale prohibitions deleted (perspective:none, 'do not apply rotateX here', 'Keeps fitScene camera (no rotateX)', 'No rotateX any more'). 4 files, +194/-95, so the CSS got smaller. selectors.py still untouched across both slices. Canon restored in d7b9a4af: mockups committed with sha256 provenance, measurements restored as docs/chef_battle/arena_mockup_spec.json marked AUTHORITATIVE, and the archived 'do not implement perspective' banner recorded as superseded by AGENTS.md section 11 v1.3.0, which permits css perspective and rotateX explicitly. G3 @ 6c4cd2d8 was a measured FAIL, reported honestly and stopped without reverting: M1 drifted 0.5340 / 0.5224 / 0.5822 across 1280 / 1920 / 390 and the worst clipping margin was -21.72 / -25.29 / -14.72 px, i.e. the stands were cut off. Bolt's diagnosis: a constant rotateX cannot produce a drifting ratio, so the scale-dependent term had to be the FIXED perspective distance, whose distortion scales with the ratio of that distance to the rendered size; and the negative margin had a second cause, fitScene sizing to the rank floor while the stands sit at 1.60x it, so they stuck out by construction. G4 @ 671d0dcd removed perspective entirely, kept rotateX(56deg), and refit by the projected stands: M1 became 0.5592 at every width with a desktop drift of 8.8e-8, which is cos(56 deg) to four places, and margins turned positive at +12.78 / +19.17 / +3.89 px. The prediction held, so the cause was found rather than tuned around. G4 left M3 at 1.7541 and Cursor stopped instead of forcing it. Bolt resolved it from the spec: stands_outer_radius 640px / 0.50 of frame was measured on the mockup as the OUTERMOST VISIBLE EXTENT, so the quantity comparable to 1.60 is the projected bounding box, not the construction radius; putting seat CENTRES at 1.60 makes the ovals overhang by half a seat. G5 @ 1187a8c3 shrank the construction radius to 1.60*1.60/1.7541 = 1.4594 and landed M3 = 1.6015, with both pre-declared corroborations hit: M1 unchanged at 0.5592 and M2 risen 0.5587 -> 0.6119 in the predicted direction. Hit testing passes front and back, overflowX is 0 at 390.",
             "blockers": ["Deploy requires explicit Owner approval. G1, G2, G4, G5 and this board go out together as one deploy. Not requested yet: the numbers pass but the frame does not yet resemble the mockup, and shipping a foundation that looks worse than the target is half a delivery. See 3I.",
                          "M2 sits at 0.6119 against the spec's 0.63. Left as measured rather than tuned: it is an OUTPUT of fitting the stands to 0.98 of the frame, not a free parameter. Revisit only if the stands change extent."]},
            # 3I is the other half of the resemblance gap, and the larger half.
            # generate_crowd_faces.py has existed with a live API key for weeks,
            # yet only THREE crowd portraits were ever generated for 290 seats,
            # cycled by _arena_render_ring.html lines 76-78. The mockup's stands
            # are a dense mass of hundreds of distinct people. No gradient, tone
            # pass or shadow can fake that, which is why colour iteration could
            # never close the gap however many releases it consumed.
            {"id": "3I", "title": "Crowd assets: real faces in real seats (A2/A3/G6)", "owner": "ArenaFront (generation); Cursor (compositing); Bolt (acceptance)",
             "status": "IN PROGRESS — 96 faces are ON PRODUCTION and WIRED to the seats, and the stands still read as grey dots. Not accepted.",
             "requirements": ["Paid generations are capped by the Owner per batch and the cap is absolute, including retries after a 429. The organisation limit is 5 images per minute; back off, never exceed the cap.",
                              "Style is now canon and locked: warm key light, dark background, consistent framing and distance, real spread of age and ethnicity, 96px, webp.",
                              "Head orientation must vary. Frontal-only portraits read as a GRID at 290 seats however many identities exist, so identity count alone is not variety.",
                              "Seat-to-face assignment must be deterministic: the same seat always gets the same face across reloads.",
                              "Adding faces must not move the geometry. M1 stays 0.5592 and M3 stays 1.60 +/- 0.05 after compositing, or it is a finding.",
                              "Density is a contract, not a dial: rings 9/10/11/12, segments 40/48/56/64, 208 stand seats. Report any delta between the render and these numbers; do not quietly increase them."],
             "evidence": "A2 @ 3bc8ff30 on agent/arenafront/a2-crowd-faces-24, contact sheet @ 9be67611: exactly 24 generations, face-01..face-24.webp plus a2_contact_sheet_24.png, gpt-image-1 quality=low 96px, 3.0-4.0 KB each. Verified by Bolt on disk, not from the report: 24 files present and 24 distinct sha256, so these are 24 people and not one template in variations. One 429 was hit and absorbed with backoff without exceeding the cap. Owner viewed the contact sheet and ACCEPTED the style as crowd canon on 2026-07-25, choosing a 96-face full run WITH head rotation over the plain 96 both agents proposed, on Bolt's finding that all 24 faces look dead-on into the camera at the same scale and would still read as a grid.",
             "blockers": ["The render still draws grey dots. 24 portraits exist on origin and none are in the frame; this is the single largest remaining cause of the gap to mockup 01. Order G6.",
                          "Visual weight is inverted against the mockup: a large bright floor with four thin rings of small grey dots outside it, where the mockup has a dense dark mass of people around a calmer floor. Weight comes from density and value, not radius, so the correct 1.60 ratio does not fix it by itself.",
                          "A stray grey translucent panel with straight edges paints under the scene, visible in the lower corners of ops/audits/arena_g5_1280x720.png. Cursor must identify what draws it before anything is removed."]},
        ],
    },
    {
    ...
    },
    {
        ...
        # oversight.
        "blockers": ["Sequentially dependent on Stage 3 completion (3D still open on Owner D3; 3G R5 on D2) and on the Stage 4 integration commit, which does not exist yet."],
...
def _arena_build_context():
    ...
    return {
        ...
        "last_verified": "2026-07-27T08:20:00.000Z",
