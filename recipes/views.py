...
ARENA_DESIGN_TASKS = [
    ...
    },
    {
        ...
        "evidence": "TWO OF THREE GAPS CLOSED, THE THIRD NEEDS AN OWNER DECISION - shipped 2026-08-16 by Bolt, card stays open. GAP ONE, CHALLENGES: CLOSED. New check_owner_not_in_battle() in services.py refuses a challenge in BOTH directions - he cannot be challenged and his account cannot issue one - wired into challenge_create() ahead of the rank matchup, so it holds against a hand-crafted POST naming his pk and not merely against the dropdown. The form's queryset excludes him too, but that is presentation and the card's acceptance says so explicitly. GAP TWO, COMPETITIVE AGGREGATES: CLOSED, and enumerated rather than patched where it came to mind - every selector that ranks chefs was read. Excluded from get_rankings, get_top_profiles, get_crown_ladder, get_crown_streak, season_service's standings write and the season_leaderboard view. New _uncompeting_slugs() helper, kept DELIBERATELY SEPARATE from the existing _hidden_bot_slugs(): bots hide behind a switch that can be turned back on (ARENA_SHOW_EMULATION_BOTS), the Owner's exclusion is permanent with no switch, and one function per reason means neither can be flipped while trying to change the other. New is_owner_author() is the author-side twin of is_immortal(), both reading settings.OWNER_SLUG so the two halves cannot drift. GAP THREE, THE CLAN REPUTATION BLESSING: NOT BUILT, and deliberately not designed by an agent. There is no clan reputation field anywhere - ChefBattleProfile.reputation is per-chef, and the only clan-level points are ClanContribution, which feeds ClanSeasonStanding. Routing a blessing through ClanContribution would make his clan win, which is exactly the competitive influence this card forbids, so the mechanism is a product decision and it is his. Options put to him: a new display-only Clan.reputation, a visual mark with no number, or his own formulation. EVIDENCE: 9 new tests green, including one proving his hand-set rating of 9999 no longer heads the ladder, one proving two ordinary chefs can still fight, and one proving is_immortal() still answers True for him - this card ADDS an exclusion and does not trade the protection away. Full chef_battle suite green on local PostgreSQL, 8 workers, exit 0. Nothing was touched on his account: the tests take the author migration 0020 already created rather than inventing a second one.",
    },
    {
