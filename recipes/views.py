...
ARENA_DESIGN_TASKS = [
    ...
    },
    {
        ...
        "evidence": "SHIPPED v2.5.1034. A mid-transfer chargeback sets the locked payout to PAID_DISPUTED, keeps its rewards reserved, and writes an immutable reconciliation event; a blocking-transfer PostgreSQL regression proves it cannot finish as ordinary PAID.",
    },
    {
        ...
        "evidence": "SHIPPED v2.5.1034. TokenLot and TokenSpendAllocation record locked FIFO provenance for purchases and gifts; historical unallocated balance is represented explicitly as LEGACY rather than attributed by timestamp. Regression coverage proves refunding order 1 does not touch a gift funded by order 2.",
    },
    {
        ...
        "evidence": "SHIPPED v2.5.1034. TokenOrder records cumulative refunded cents and clawed tokens; verified refund payloads debit only the newly refundable proportional delta and repeated cumulative values are idempotent.",
    },
    {
    ...
    },
    {
        ...
        "status": "DONE", "owner": "Bolt",
        "files": "chef_battle/models.py; chef_battle/services.py (ingredient combat); battle setup and combat UI; notifications; tests; docs/chef_battle/ingredient_combat.md",
        "depends_on": "T12",
        "action": "Before Stage 1 starts, both chefs secretly lock exactly 2 key ingredients in their own declared recipe. Stage 1 determines one winner. Only that winner receives exactly 3 shots at the loser's ingredient list; the loser never shoots and is protected only by the 2 blocks placed before the battle. Reveal each targeted lock when it is hit, preserve blocked ingredients, remove unblocked hits, and record and notify the final result. Define one pre-battle lock deadline and the Stage 1 action deadline/no-response policy; do not build sequential loser-then-winner 48-hour windows.",
        "visible_result": "Both chefs prepare two hidden defences before the fight; after Stage 1, its winner fires three times and the loser only defends.",
        "acceptance": "Tests prove: both chefs must place exactly two hidden blocks before Stage 1; neither can see the opponent's blocks; only the Stage 1 winner may fire; the winner gets exactly three shots; the loser cannot fire or change blocks after Stage 1; blocked hits survive and reveal that block, unblocked hits eliminate the ingredient; draw/no-response/repeated-submit paths are deterministic and idempotent.",
        "forbidden": "No post-result lock placement; no shots for the Stage 1 loser; no sequential 48-hour loser-lock then winner-shot workflow; no exposure of hidden blocks before a hit or final reveal.",
        "evidence": "SHIPPED. HALF OF THIS TICKET WAS ALREADY BUILT AND NOBODY HAD NOTICED: BattleIngredient.is_key with KEY_COUNT=2, written by declare_menu, already IS both chefs secretly blocking exactly two ingredients before Stage 1 - declare_menu refuses a re-declaration outright and the battle only reaches ACTIVE once both chefs have declared, so \"before Stage 1, immutable, hidden\" was structurally guaranteed and needed no new code, no new model and no migration. What was actually wrong was only the post-combat half, and it was wrong in both halves of its own design: the LOSER placed two locks AFTER he had already lost (the sequential shape the ruling forbids), and he placed them on the raw TEXT LINES of his submitted recipe - a completely different data source from the declared menu the fight was fought over. Two lock concepts, two answers to \"is this ingredient protected\". Now: IngredientLock and place_ingredient_lock are DELETED, IngredientShot.target_index (a recipe line number) becomes target_ingredient, a FK to the BattleIngredient row itself, and bounced=True IS the reveal the ruling asks for. One shared eliminate_ingredient() helper serves both round combat and the winner's shots - a conditional UPDATE with is_key=False and is_eliminated=False in the WHERE clause, so a block can never be eliminated and nothing is counted twice, whatever two concurrent callers believed. The 15-minute shot window is the Owner's own number, set 2026-08-15; sweep_ingredient_penalty_deadlines() advances the battle to COOKING with however many shots were fired, so a winner who closes the tab cannot hold his opponent's battle open. TWO REAL DEFECTS FOUND BY THIS CARD, both fixed inside it. First: get_surviving_ingredients returned the WINNER's list completely unfiltered and only ever filtered the loser, and only by IngredientShot.bounced - never by BattleIngredient.is_eliminated, which round combat has been setting on EITHER side all along, so a chef who lost ingredients in Stage 1 cooked with them back. Second: THE EMULATOR NEVER DECLARED A MENU. Its own note said \"menus declared\" while it only submitted entries, which is why production carried five old lock/shot rows against ZERO BattleIngredient rows; it now really declares, which also forced the correct step order - reveal_entries_if_ready is no longer called there, because on an ACTIVE battle whose entries carry dish_submitted_at it takes the other branch and jumps straight to VOTING, skipping combat, biathlon and cooking. Migration 0098 drops the superseded rows rather than backfilling: measured on production first - 2 locks and 3 shots, all five on battle 14, COMPLETED, both chefs emulation bots, zero battles in INGREDIENT_PENALTY and zero BattleIngredient rows to point at. No real chef history, no money, no audit chain. RED ON MAIN WHEN THIS CARD ARRIVED, NOT MINE, FIXED ON THE WAY PAST: v2.5.1062 added a 150ms setTimeout before the centre click navigates, and two guards counted timers instead of judging them, so a deliberate post-click flourish read as a first-paint readiness gate. Both now name what they allow and still fail on a third. Evidence: 1089/1089 chef_battle tests green on local PostgreSQL with 8 workers; 15 of them new for this card; arena acceptance, board and image-weight suites green; manage.py check, makemigrations --check --dry-run and git diff --check clean.",
    },
    {
        ...
        "evidence": "SHIPPED v2.5.1034. Battle status has a formal transition matrix, PostgreSQL trigger enforcement, a transaction-local audited operator override and an ORM writer audit. Admin lifecycle/result fields are read-only. The 345-test local PostgreSQL Arena gate passed.",
    },
    {
        ...
        "evidence": "SHIPPED. nonce=\"{{ request.csp_nonce }}\" added to the exact three gaps confirmed open. Repo-wide sweep (not just the three named files) of every template under templates/ found zero other bare <script> tags - the 36+ other chef_battle templates already carried the nonce, this was a three-file omission, not a pattern gap. config/csp_middleware.py's script-src carries no unsafe-inline and was not touched.",
    },
    {
        ...
        "evidence": "SHIPPED. CORRECTION TO THE TICKET'S OWN PREMISE: BattleMoveTransaction.reason is a live field (chef_battle/models.py:673, read by the command at its own line 51) - it was never removed. The real, sufficient reason for deletion is AGENTS.md section 20: the command unconditionally rewrote the Owner's protected ChefBattleProfile (rank=EXECUTIVE_CHEF, rating=9999, infinite_moves=True) with zero guard, dry-run or confirmation - exactly the privileged-account tooling section 20 forbids building. Deleted outright, not disarmed: repo-wide grep confirmed zero callers (no cron, no other command, no test exercised it). manage.py check clean.",
    },
    {
    ...
    },
    {
        ...
        "evidence": "SHIPPED. Both templates deleted; the guide-only CSS block (static/css/chef_battle.css, .battle-guide-search/-sections/-section__*/-grid/-card*) deleted with them, confirmed by repo-wide grep to be unique to the deleted page. Left untouched: .battle-guide-list/-note (live on challenge_form.html, similar name, different thing) and .battle-home__wordmark/-pre-title/-title*/-divider* (consumed by home.html's own separate inline markup, not by the deleted partial - confirmed by reading home.html, the wordmark there was never {% include %}'d from _battle_wordmark.html). The chef_battle:guide URL and its redirect to rules (views.py battle_guide()) are untouched and still linked from hall_of_fame.html. Zero test coverage existed for either deleted file.",
    },
    {
        ...
        "evidence": "SHIPPED. Owner brief 2026-08-12, ticket 17. Card written to GreenBear and handed to Bolt by GreenBear himself on the Carpet (#3515) once AA6/AA7/AA8 was his last package; this gate runs over both agents' work, which is why it runs once and last. THE FULL SUITE, ONE RUN, NOTHING EXCLUDED: engine django.db.backends.postgresql, database culineire, Django 5.2.13; manage.py test --noinput --parallel 8 on an 8-core workstation, and nine test databases were created and destroyed - eight worker clones plus the parent - which is the worker count made visible rather than asserted. Found 2133 tests, ran 2133 in 1029.795s (17m10s), OK with skipped=3, exit code 0. Zero failures, zero errors, ZERO PRE-EXISTING FAILURES: nothing was deselected, weakened or marked expected to make this green. THREE DIFFERENT TEST COUNTS EXIST AND THEY ARE THREE SCOPES, NOT THREE ANSWERS: 2133 is the whole project, 1094 is chef_battle alone (T11's figure), and GreenBear's pre-release 1344 is a third selection I did not reproduce and do not claim to explain. The run's log prints 'eu: failed / records=0' and 'network down' from the sanctions-screening tests driving their own failure paths; those are asserted outcomes inside a suite that finished OK, not breakage. GATES: manage.py check - no issues, 0 silenced; makemigrations --check --dry-run - No changes detected, so T11's 0097/0098 are committed and match; git diff --check clean. THE BROWSER SMOKE, 21/21 SURFACES, 0 FAILED, 0 UNRESOLVED NAMES: GETs only against the local development database, no POST and no write of any kind, so no production write and no demonstration data anywhere. Every address comes from reverse(), never hand-typed - the first pass guessed six paths and reported them as 404s, which reads exactly like a broken page and was not one, and the second pass guessed three URL NAMES that do not exist (arena_master_console, battle_state, changing_room with a pk); a name either resolves or the smoke fails loudly, which is how those three were caught and corrected to master_console, battle_state_poll and the two real changing-room routes. 200: arena, rules, rankings, challenge list, challenge create, clan create, hall of fame, token shop, battlefield progress, changing room, cooking moderation, moderation panel, arena build board, battle detail, biathlon, and the combat state poll. 403, correct and not a defect: cooking submit and the battle changing room - the smoke user is not a participant in that battle. 404, correct and not a defect: master console, live arena progress and live arena preview - DG-01 refusing a staff+superuser who is not the Owner, which is the gate working. THE ONE ITEM THE SMOKE CANNOT ANSWER, SAID PLAINLY: the console reconciliation display cannot be RENDERED by me. DG-01 admits OWNER_SLUG, or superuser plus RecipeAuthor.has_arena_console_access; AGENTS.md 1a/18 makes the Owner's account untouchable and section 20 reserves that flag to him alone, so granting myself either would be the violation rather than the evidence. What renders it instead is this same green run, where an Owner exists only inside an ephemeral test database: ArenaMasterConsoleAccessTests, ArenaMasterStateTests, ArenaMasterActionTests, ConsoleArenaMirrorTests, and for the reconciliation display specifically ArenaMasterEconomyTests.test_flows_reconcile_to_ledger, which asserts the console's headline totals reconcile against the same signed ledger they are drawn from. Per AGENTS.md 17.14 nothing here is a claim about how any page LOOKS on production: this is status codes, routing and absence of tracebacks. The throwaway smoke script and its local-only t17-smoke user were both deleted after the run.",
    },
    {
    ...
    {
        "id": "T22", "group": "Owner Authority (Owner ruling 2026-08-15)", "title": "GreenBear is present on the Arena and outside the competitive reckoning",
        "status": "DONE", "owner": "Bolt",
        "files": "chef_battle/forms.py (the opponent queryset); chef_battle/services.py (check_owner_not_in_battle, is_owner_author); chef_battle/selectors.py (_uncompeting_slugs and every competitive aggregate); chef_battle/clan_selectors.py (clan_is_blessed); static/css/chef_battle.css (.rank-aura--blessed); tests",
        "depends_on": "T18",
        "action": "Build the half of the Owner-avatar contract that T18 deliberately did not claim. HALF OF IT IS ALREADY IN AND MUST STAY: is_immortal() protects his account at the three places anything could be taken from it - the post-battle penalty (services.py:224), the rank demotion (services.py:414) and the 15-move refusal charge (services.py:723). The missing half is the exclusion, and three concrete gaps were verified in the code on 2026-08-16. ONE: he is selectable as an opponent - forms.py:31 builds the opponent queryset as every RecipeAuthor with a user, minus the challenger, with no exclusion of the Owner, so any chef sees him in the dropdown. TWO: he stands in the public ladder - get_rankings() filters on enrolled_at only, while his rank is hand-set (migration 0020 EXECUTIVE_CHEF, 0025 infinite balance), so a ladder keyed to wins carries an account that did not earn its place. THREE: the clan reputation blessing does not exist - a repo-wide search over code and templates returns nothing. What must remain true after the work: he still APPEARS on the Arena, and he may still join clans and alliances cosmetically.",
        "visible_result": "GreenBear stands on the Arena floor and may wear a clan, and no chef can challenge him, meet him in a battle, or find him anywhere in the rankings or any other competitive total. His clan gains reputation from his presence and nothing else.",
        "acceptance": "He cannot be selected as an opponent and a challenge naming him is refused server-side, not merely hidden from the form; he is absent from get_rankings() and from every competitive aggregate, proved by enumerating them rather than by fixing the two that came to mind; he is still rendered on the Arena and still able to hold cosmetic clan and alliance membership; the blessing adds reputation and touches no competitive number; all three is_immortal() protections still pass unchanged. Focused PostgreSQL tests green.",
        "forbidden": "DO NOT TOUCH THE ACCOUNT ITSELF - AGENTS.md sections 1a, 18 and 20. Do not write is_staff, is_superuser, has_bearseeker_privileges or has_arena_console_access by any means, and do not build a tool that makes it easier. Do not alter his hand-set rank, his balance, his profile or his page. Do not remove or weaken any is_immortal() protection - this card adds an exclusion, it does not trade one for the other. Do not generalise the hardcoded owner slug that section 18 protects; use the existing settings.OWNER_SLUG check that services.py already names.",
        "evidence": "ALL THREE GAPS CLOSED. GAP ONE, CHALLENGES: check_owner_not_in_battle() refuses a challenge in BOTH directions and is wired into challenge_create() ahead of the rank matchup, so it holds against a hand-crafted POST naming his pk and not merely against the dropdown. GAP TWO, COMPETITIVE AGGREGATES: closed by enumerating every selector that ranks chefs rather than patching the two that came to mind - get_rankings, get_top_profiles, get_crown_ladder, get_crown_streak, the season standings write and the season_leaderboard view. GreenBear's T31 later found three more read paths this enumeration missed (the arena centre stage, the crown_holder page, the Master Console) and closed them in v2.5.1080; that is what an enumeration is for and it is recorded rather than glossed. New _uncompeting_slugs() is kept deliberately separate from _hidden_bot_slugs(): bots hide behind a switch built to be turned back on, this exclusion is permanent. GAP THREE, THE BLESSING - AND IT WAS THE WRONG REQUIREMENT ALL ALONG. The card asked for a 'clan reputation blessing' and three times I asked the Owner to specify a reputation mechanic. He told me to go and read what he had actually written, and he was right: THOSE WORDS ARE NOT HIS. A repo-wide trace found 'clan reputation blessing' in exactly two places - Ember's T18 handoff summary, and this card quoting it. His canonical rules never say it. What he did write about clans (clans_alliances_rules.md, his own worked example) is that GreenBear FOUNDS a clan like any chef. HIS RULING, 2026-08-17: the blessing is the AURA. If he stands in a clan, the whole clan's aura burns gold instead of wearing its top chef's rank aura. Cosmetic only - no rating, no reputation, no points, no advantage of any kind, 'only the knowledge that this is a god's clan'. Reputation, he said, is a human matter and not a field: a god's standing is that he can delete and punish accounts that break the rules. Built as clan_is_blessed() plus a CSS class - and a Clan.reputation column, which was one word away from being written, is not: it would have been a number ranking clans by whose side he took, the exact competitive influence this card forbids. Blessing and top-chef stay different questions: he blesses a clan, he never represents it, and the T22 exclusion keeps his hand-set rank from making him its face. EVIDENCE: 16 tests across the card - 9 for the exclusion (including his hand-set 9999 no longer heading the ladder, two ordinary chefs still able to fight, and is_immortal() still True so the protection was not traded away for the exclusion) and 7 for the blessing (a pending request blesses nothing, leaving takes it with him, the points ledger and standings are untouched, and Clan has no reputation field). THE LESSON, recorded because it cost three exchanges: I built the whole of T22 on verifying sources, and then spent three rounds asking the Owner to specify a requirement I had never traced to him."
    },
    {
        "id": "T23", "group": "Chef Battles roadmap (Owner list, 2026-08-16)", "title": "Clans - ALL OF IT - SEASON 2",
        "status": "DEFERRED", "owner": "unassigned",
        "files": "chef_battle/models.py (Clan, ClanMembership, ClanContribution already exist at models.py:2097-2222); a new battle-format field or model; chef_battle/services.py; chef_battle/views.py; docs/chef_battle/clans_alliances_rules.md (canonical); tests",
        "depends_on": "none",
        "action": "The battlefield board (chef_battle/views.py:227) has carried this as a bare pending line since Phase 6 - 'Team-based battle formats after individual battle mechanics are stable and tested' - with no design underneath it. Clans and memberships already exist and are stable; individual battles are stable and tested (2133 green, T17). Design and build team-based battle formats on top of them: how a clan fields a team, how a team battle scores against the existing Battle state machine, how ClanContribution and ClanSeasonStanding interact with a team result.",
        "visible_result": "Two clans can field teams against each other and the result affects clan standing, not just individual chef records.",
        "acceptance": "Not yet defined - the first step is a design note against clans_alliances_rules.md, not code. No design decision is an agent's to make alone; state the open questions and get his ruling before building.",
        "forbidden": "Do not invent a scoring model without his sign-off. Do not touch is_immortal() or any T22 exclusion while building this.",
        "evidence": "ALL CLAN WORK IS SEASON 2 - Owner, 2026-08-17, WIDENED THE SAME DAY. He first deferred T23 (clan and team battles), restating what he had already written on 2026-07-14 in clans_alliances_rules.md section 2: Season 1 carries the alliance entity, the clan-alliance link and the call-an-ally hook; the full cuisine-vs-cuisine mechanic is Season 2. Then he widened it to CLANS ENTIRELY. That takes the Season-1 call-an-ally hook with it - the one unbuilt piece of the clan foundation, which this card had named and left unassigned as an open question to him. It is not open any more, it is deferred. WHY IT WAS NEVER A SMALL CHANGE ANYWAY: Battle is hard-wired to two chefs - challenger and opponent as foreign keys with a database CHECK constraint forbidding them to be equal - and everything downstream assumes exactly two: combat actions per chef, opponent_for(chef), declared ingredients per side, the vote split, the scorer. A team format changes the shape of the model rather than adding to it. And by his own rules a fight with allies becomes a battle OF A CUISINE rather than of a single recipe, which is a different product, not an extension. WHAT IS ALREADY BUILT AND NOW SITS IDLE UNTIL SEASON 2: the Clan and ClanMembership models, the request-approve join flow, the ClanContribution ledger, ClanSeasonStanding, the Alliance entity and the clan-alliance link, the clan pages, and this month the clan aura (v2.5.1115) and the Owner blessing (v2.5.1123). MEASURED ON PRODUCTION, and it is the context for his call: ZERO clans, ZERO alliances, ZERO clan memberships against four battles. Nothing built around clans has been seen by a single person.",
    },
    {
        ...
        "acceptance": "CORRECTED 2026-08-17 - this criterion described the wrong enforcement point and the shipped code is deliberately different. It said combat RESOLUTION refuses to resolve a round for a chef holding an unused gift; that would deadlock the battle, because resolution runs when BOTH chefs have submitted and neither could then unstick it. The guard belongs where the chef can still act: submit_combat_action refuses the ACTION, with a message naming the artifact, so the chef simply picks it and carries on. Actual acceptance: an action that ignores an unspent BATTLE_GIFT artifact of the matching effect type is refused at submission; a gifted defence artifact does not block an attack; is_applied is set at consumption, not receipt, exactly one row per use; AppreciationGift stays provably inert to any combat number. Focused PostgreSQL tests green.",
    ...
    },
    {
        ...
        "status": "DONE", "owner": "Bolt",
        "files": "static/js/arena_render.js (the two cell-centre maps and the move in appendOccupant); static/css/arena.css (.arena-occupant--moving). NOTE: arena-teleport-flash, which this card originally pointed at, is the centre-stage ARRIVAL ring and is a different feature - untouched. The frozen camera architecture is untouched (ARENA_BATTLE_PLAN)",
        "depends_on": "B2, B3 (both DONE - static relocation already ships)",
        "action": "The battlefield board's own line (views.py:355) already names the sequencing: 'ship static relocation first (done via B2+B3), animate second per original handoff advice.' Static relocation is done. This is the animate-second half: an SVG transition between the ten-second polls (AA1 corrected the poll cadence from a stale '20s' figure) rather than an instant jump. arena-teleport-flash already exists as a class name at arena_render.js:2474 - verify what it currently does before assuming it is unbuilt.",
        "visible_result": "A chef relocating on the Arena floor animates between positions instead of jumping.",
        "acceptance": "SETTLED 2026-08-17, after the investigation this field was waiting on. arena-teleport-flash is a DIFFERENT feature - the centre-stage arrival ring - and is untouched. Acceptance as built: a chef whose ring cell changes between polls travels to it instead of jumping; a chef appearing for the first time does not slide in from the origin; the movement is a CSS transition scoped to chefs that actually moved, so no timer is spent and AN20's pinned renderer budget is unchanged; reduced motion switches it off. The frozen camera and the octagon region are untouched (ARENA_BATTLE_PLAN, THE FROZEN ARCHITECTURE).",
        "forbidden": "No third Arena stylesheet. No fake pairs or forced animation for demonstration.",
        "evidence": "SHIPPED 2026-08-17. FIRST, WHAT THE EXISTING HOOK TURNED OUT TO BE: the card said to check arena-teleport-flash before assuming this was unbuilt, and it is NOT this. That is the centre-stage ARRIVAL flash - an expanding ring fired when a pair takes the centre - and it stays exactly as it was. B5 is the relocation between ring CELLS, and none of it existed. THE PROBLEM: the occupants layer is wiped and rebuilt from scratch on every poll, so a chef who changes cell simply vanishes from one and appears in another. To animate a move you have to know where the chef WAS, and nothing recorded it. Two maps now do - previousCellCentres, read while this paint draws, and lastCellCentres, written as each occupant lands - swapped inside bind() before a single occupant is appended. Two rather than one deliberately: a single map would be overwritten mid-loop and every chef would compare against himself, so nothing would ever animate. THE ANIMATION RUNS BACKWARDS ON PURPOSE. The chef is drawn in his NEW cell first, then offset back to where he was and released. Going this way round means the clip-path, the avatar and the online dot are all already correct for the destination; only the group slides. Animating towards the new cell would have needed the destination clip applied to an element still standing somewhere else. NO TIMER AND NO PER-FRAME JAVASCRIPT: the movement is a CSS transition, so AN20's pinned timer budget is untouched (verified - the renderer guard tests pass unchanged) and prefers-reduced-motion switches it off in the same place as everything else. The transition is scoped to .arena-occupant--moving and NOT to .arena-occupant, because the layer is rebuilt every poll and a transition on the base class would fire on the twenty-odd chefs who did not move, every single time. 1.6s against a 10s poll: long enough to read as travel, finished long before the next paint. ONE DELIBERATE SYNCHRONOUS REFLOW, of the kind AN20's own guard docstring allows ('each read depends on the write above it') - without it the browser coalesces setting the offset and clearing it into one recalculation and there is nothing to transition from. Its cost is bounded by chefs who actually moved: normally none, one or two on a rank change. A chef appearing for the first time has no previous position and is guarded so he does not fly in from the SVG origin, which would read as a bug rather than as movement. EVIDENCE: 6 new tests, including the map-swap ordering asserted INSIDE bind() rather than across the file - the first version of that test compared source-text positions and failed, correctly, because appendOccupant is defined above bind() and source order says nothing about execution order.",
    },
    {
        "id": "T26", "group": "Chef Battles roadmap (Owner list, 2026-08-16)", "title": "TikTok / Instagram live integration - SEASON 2",
        "status": "DEFERRED", "owner": "unassigned",
        "files": "unbuilt - no existing code in this repo integrates either platform's API",
        "depends_on": "none",
        "action": "Confirmed he wants a real live stream (arena/camera feed pushed into TikTok/Instagram Live), not a social-media link - those already exist in the site footer and needed no work. Before any code: TikTok developer account + Live Streaming API application, and a Meta Business Account + business verification + Instagram Graph API App Review. Both are external, identity-verified processes only he can start, typically 1-3 weeks turnaround, and neither platform's Live API is guaranteed on application. Sequence handed to him 2026-08-17 as docs/chef_battle/T26_LIVE_API_ACCESS_TOR.md, sent to him by email through the site's own configured mailer (chef_battle/stripe_services.py's send_mail pattern) so it's reachable from a phone.",
        "visible_result": "Not yet defined - blocked on external platform approvals he has not started collecting.",
        "acceptance": "Not yet defined.",
        "forbidden": "Do not attempt live video streaming infrastructure before the platform accounts and API approval exist - there is nothing to build against.",
        "evidence": "DEFERRED TO SEASON 2 by the Owner, 2026-08-17, the same day the access TOR was sent - not started while the platform applications are outstanding (1-3 week external turnaround each), and not urgent against a launch that does not require it. Unassigned rather than left on Bolt: nobody should be carrying a card with nothing to build. Re-open once even one platform's Live API is approved and the Owner has decided what actually streams (arena screen capture, an opt-in camera feed, or the existing photo uploads as a slideshow) - his call, not yet made.",
    },
    {
        "id": "T27", "group": "Frontend plan (Owner-approved 2026-08-16)", "title": "F1 - Arena gap list vs the design template",
        "status": "DONE", "owner": "GreenBear",
        "files": "docs/chef_battle/ARENA_F1_GAP_LIST_2026-08-16.md",
        "depends_on": "none",
        "action": "Compare the live Arena against ops/reference/design_arena/Chef Battles Arena v2.dc.html by grep against the renderer/CSS/template, not by eye. Produce a numbered list of real absences, separating them from decisions the Owner already froze.",
        "visible_result": "A written gap list; no code changed.",
        "acceptance": "Every item is checked against actual code (hit count in arena_render.js/arena.css), not assumed from a screenshot. Frozen decisions (octagon ring count, camera angle, colour palette) are named as NOT gaps, not silently included.",
        "forbidden": "Do not touch the octagon geometry, the camera, or the colour palette while compiling this list - it is read-only.",
        "evidence": "docs/chef_battle/ARENA_F1_GAP_LIST_2026-08-16.md. Panels and data already match the template (four pulse metrics, crown ladder, Recent Battle Gifts, 7-step phase rail, country+flag, rank tier key, lanterns). Six real absences found, all ambient/atmosphere, none touching frozen geometry: G1 three swaying light shafts, G2 floating dust motes, G3 gift emojis rising from the crowd, G4 a running ticker (ours is three static spans), G5 crown-holder badge pulse, G6 blinking Live Now dot. Filed as T28.",
    },
    {
        "id": "T28", "group": "Frontend plan (Owner-approved 2026-08-16)", "title": "Arena ambient atmosphere - light shafts, motes, gift rise, running ticker, crown pulse, live dot",
        "status": "DONE", "owner": "GreenBear",
        "files": "static/css/arena_atmosphere.css (sections F-K, six new keyframes); templates/chef_battle/arena.html (a decorative aria-hidden block for shafts/motes/gifts; a wrapper div around the ticker's three existing spans); chef_battle/tests.py (ArenaAmbientAtmosphereTests)",
        "depends_on": "T27",
        "action": "Build the six items T27 found absent: G1 three swaying light shafts, G2 floating dust motes, G3 gift emojis rising from the crowd, G4 the activity ticker actually scrolling instead of sitting static, G5 the crown-holder badge pulsing, G6 the Live Now dot blinking.",
        "visible_result": "The Arena hall reads as alive rather than static, without moving the octagon, the camera, or any panel's position.",
        "acceptance": "All six present, verified live in the browser against the read-only preview route (chef_battle:arena_preview_current), not assumed: 3 shafts / 8 motes / 4 gifts rendered with the correct animation-name each, the ticker track scrolling with arena-ticker-run, the three ticker span IDs unchanged so arena_deck.js's getElementById calls are untouched. prefers-reduced-motion disables all six - proved statically per-selector (ArenaAmbientAtmosphereTests asserts each selector sits inside its own @media (prefers-reduced-motion: reduce) block, this file's own established one-block-per-effect convention, not one master block). Does not touch the frozen architecture (ARENA_BATTLE_PLAN, THE FROZEN ARCHITECTURE) or any panel's measured position - the layer-ladder z-index guard (ArenaStylesheetHasNoSupersededDeclarationTests) still passes.",
        "forbidden": "No change to octagon geometry, camera values, or the accepted 1280x800 composition. No new stylesheet - extended the existing arena_atmosphere.css, the file GreenBear built for exactly this purpose (AN12).",
        "evidence": "CREATED 2026-08-16 from T27's gap list, on the Owner's approved F1->F2->F4 plan (F1 is this list; this card is F1's direct output, not F2). CSS-ONLY EXCEPT ONE TEMPLATE DECISION MADE MID-BUILD: the approved plan assumed the shafts/motes/gifts could be pure ::before/::after tricks with zero template change beyond the ticker wrapper; building it, three independently-timed shafts, eight staggered motes and four staggered gifts cannot share the two pseudo-element slots one selector gets, so a small aria-hidden decorative block (.arena-atmosphere-fx, 15 spans, zero identity, zero interaction) was added to the template instead - CSS still does 100% of the visual/animation work. VERIFIED LOCALLY: migrated the local PostgreSQL dev database, set ARENA_PREVIEW_SHARE_TOKEN and CHEF_BATTLE_ENABLED in the local-only gitignored .env, ran manage.py runserver via a new .claude/launch.json (also gitignored - nothing of this touches version control), loaded /chef-battle/preview/arena/<token>/ and inspected the live DOM: fxCount 1, shafts 3, motes 8, gifts 4, all four animation-names present and correct, ticker track scrolling. Crown pulse and the live dot could not be visually exercised on the LOCAL empty dev DB (no active battle or crown holder there), but their MARKUP was confirmed live against PRODUCTION instead, read-only, before assuming the local gap was harmless: curl against chef_battle:arena_preview_current (production's own token-gated share route, no login, no write) returned class=\"arena-live-chef arena-live-chef--crown\" and class=\"arena-live-centre arena-live-centre--crown\" - production currently holds a live crown, so G5's selectors ARE live targets today, not merely theoretical ones a future crown might reach. The dot's ::after rule was similarly confirmed to target a real id=\"arena-live-note\" element on the same page. Reduced-motion gating for both still rests on the focused tests, since neither this curl nor the local browser check can toggle a media feature. TESTED: 8/8 new focused tests green (ArenaAmbientAtmosphereTests) plus the adjacent Arena guard classes (ArenaStylesheetHasNoSupersededDeclarationTests, ArenaReadinessLifecycleTests, ArenaLifecycleMechanismCountTests, ArenaCameraIsOwnedByTheOctagonTests) on local PostgreSQL parallel 8, all green; manage.py check clean; git diff --check clean.",
    },
    {
    ...
    },
    {
        ...
        "status": "DONE", "owner": "GreenBear",
        "files": "chef_battle/views.py (_arena_center, crown_holder); chef_battle/selectors.py (get_master_state); chef_battle/tests.py (OwnerIsPresentButOutsideTheCompetitionTests)",
        "depends_on": "T22",
        "action": "T22 (Bolt, 76d29aad) enumerated the competitive aggregates the Owner must be absent from - get_rankings, get_top_profiles, get_crown_ladder, get_crown_streak, season standings - and get_crown_streak already carried the exclusion. THREE separate read paths onto the same crown_until field did not: _arena_center()'s own crown_holder query (the arena's centre stage), the dedicated crown_holder view (G13's 'who wears the crown' page and its past-winners list), and get_master_state()'s crown query (the Master Console). Found 2026-08-16 verifying T28 in the browser: production's crown_until was set a decade out on the Owner's own account for lamp-calibration testing, so without this exclusion the arena centre showed him as Crown holder while the adjacent, already-fixed Crown Streak metric showed 0 for the same reign - a visible self-contradiction on one page.",
        "visible_result": "The arena centre, the crown-holder page and the Master Console never show the Owner as crown holder, consistent with every other competitive surface T22 already covers.",
        "acceptance": "All three queries now exclude _uncompeting_slugs() the same way get_crown_streak already does. Three new focused tests in the existing T22 test class prove it directly against his real profile row, not a stand-in - including one against get_master_state() itself, asserting state['arena']['crown_holder'] is None.",
        "forbidden": "This does NOT widen into a general Master Console filter - every other section (enrolled/online/suspended counts, battles, moderation, voting, viewers, economy) stays exactly as raw as his 2026-08-16 ruling requires. Only the one field that can never legitimately hold his slug (crown_until - T22 already makes a real win structurally impossible for him) is excluded there too.",
        "evidence": "TWO OWNER RULINGS ON THE SAME DAY, IN SEQUENCE, AND THE SECOND CORRECTS THE FIRST - recorded because reversing a decision is not the same defect as never making one. First: 'просто добавь эти новые правила в правило T22' (add these new rules to T22) with the account itself untouched - shipped as the code-only fix. Told this would leave the Arena and the Master Console visibly disagreeing while his crown_until still held a real value (the Arena would hide it, the console would show it, because the console's own 2026-08-16 ruling makes it deliberately unfiltered), he asked to understand why on the console specifically a real chef's crown and his own crown were being treated differently, and on reflection ruled: 'Если кто-то снова случайно впишет мне тестовую дату в crown_until - Мастер-консоль должна тоже прятать её' (if a test date is ever set again, the console must hide it too). THE REASONING THAT RESOLVED THE APPARENT CONTRADICTION: for every real chef the Arena and the console were never going to disagree - _uncompeting_slugs() only ever names the Owner. And because T22 already makes a real win structurally impossible for him (check_owner_not_in_battle refuses every challenge naming him in either direction), any future crown_until value on his row can only ever be leftover test fixture data, never a genuine event - so hiding it from the console is not filtering truth, it is refusing to display an artifact as if it were truth. SHIPPED v2.5.1114, bundled with T28's ambient-effects release from the same investigative session, alongside a one-off, read-before/write/read-after data clear of the live 2036 test value on his own explicit instruction ('Убери мою корону (crown_until)') - account-owner authorization for his own account, not an agent's unilateral decision; config/release_journal.py v2.5.1114 records the before/after values.",
    },
    {
...
ARENA_RELEASE_STAGES = [
    ...
    },
    {
        ...
        "commit": "f3fdefdb / production v2.5.1127",
...
def _arena_build_context():
    ...
    return {
        ...
        # 2026-08-17: DERIVED, not a constant. This was the literal string
        # "NOT READY" and would have read NOT READY for ever - including the
        # day after launch - which is the same defect class as the stale
        # version numbers T15 removed: a headline metric that cannot change.
        # It is also carefully NOT a claim that the product may launch. It
        # reports only what this board actually knows: whether its own three
        # stages are finished. The launch decision is the Owner's and lives
        # in the Chef Battles specification, not here.
        "release_readiness": (
            "STAGES COMPLETE"
            if all(st["status"] == "DONE" for st in ARENA_RELEASE_STAGES)
            else "NOT READY"
        ),
        ...
        "last_verified": "2026-08-17T11:00:00.000Z",
