...
ARENA_DESIGN_TASKS = [
    ...
    },
    {
        ...
        "files": "static/css/arena.css; static/js/arena_render.js; templates/chef_battle/arena.html (the .arena-activity-ticker markup, currently three static spans)",
        "depends_on": "T27",
        "action": "Build the six items T27 found absent: G1 three swaying light shafts (template's shaftSway keyframe), G2 floating dust motes (moteFloat), G3 gift emojis rising from the crowd (giftRise), G4 the activity ticker actually scrolling (tickerRun) instead of sitting static, G5 the crown-holder badge pulsing (crownPulse), G6 the Live Now dot blinking (liveDot). CSS/JS-additive to the existing DOM; no restructure.",
        ...
        "acceptance": "All six present and correctly cued off live server state where the template's are decorative-only (the ticker must scroll real activity, not template placeholder text). prefers-reduced-motion disables all six, matching the template's own rule and this project's existing pattern. Does not touch the frozen architecture (ARENA_BATTLE_PLAN, THE FROZEN ARCHITECTURE) or any panel's measured position.",
        "forbidden": "No change to octagon geometry, camera values, or the accepted 1280x800 composition. No new stylesheet - extend arena.css.",
        "evidence": "CREATED 2026-08-16 from T27's gap list, on the Owner's approved F1->F2->F4 plan (F1 is this list; this card is F1's direct output, not F2).",
    },
    {
...
ARENA_RELEASE_STAGES = [
    ...
    },
    {
        ...
        "commit": "76d29aad / production v2.5.1111",
