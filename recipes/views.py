    ...
    },
    {
        ...
        "workstreams": [
            {"id": "3A", "title": "Backend access and initial-render context", "owner": "Bolt",
             "status": "IMPLEMENTATION COMPLETE — UNVERIFIED AND NOT DEPLOYED",
             "requirements": ["Registered-author dark-launch access branch removed; Arena visibility is staff/superuser-only in the implementation commit.",
             ...
             "evidence": "Owner Bolt. Source branch impl/arena-access-and-context, source commit ce43242b65fc3d0ca8a259544ab4d1ffb9f7eaf8, branch pushed: yes. Merged to main: no. Deployed to production: no. Tests run: zero by owner policy (no test execution while proper distributed test capacity is unavailable). Static verification: owned-file scope confirmed (chef_battle/access.py, chef_battle/views.py, chef_battle/tests.py only) and git diff --check passed; temporary .env removed from the isolated worktree. CoWork evidence: messages 1960 (owner), 1961 (GreenBear), 1962 (Ember). Next required action: integrate this backend commit into current main, deploy it, live-verify staff/superuser-only access and the initial-render context, then complete distributed verification when GreenBear returns. UNVERIFIED until Stage 5."},
            {"id": "3B", "title": "Frontend accessibility and token migration", "owner": "Bolt (integration/deploy); original owner GreenBear; implementation Ember",
             "status": "DEPLOYED — SERVED ASSETS VERIFIED — INTERACTIVE STAFF QA PENDING",
