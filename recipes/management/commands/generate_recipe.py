...
def _normalise_recipe_payload(payload: dict, dish_name: str, status: str) -> dict:
    ...
    return {
        ...
        "short_description": _s(_to_text_lines(payload.get("short_description"))),
        ...
        "method": _s(method),
        "tips": _s(_to_text_lines(payload.get("tips"))),
        "irish_context": _s(_to_text_lines(payload.get("irish_context"))),
        "author_commentary": _s(_to_text_lines(payload.get("author_commentary"))),
...
def _generate_step_photos_from_plan(recipe: Recipe, steps: list[dict]) -> list[RecipeImage]:
    ...
    for gallery_pos, step in enumerate(steps[:3], start=1):
        ...
        try:
            ...
            _validate_image_bytes(image_bytes, context=f"step {step_num}")
        ...
        img.image.save(f"step{gallery_pos}-{recipe.slug[:30]}{ext}", ContentFile(image_bytes), save=False)
        img.save()
        created.append(img)
    return created


def _validate_image_bytes(image_bytes: bytes, context: str = "") -> None:
    """Raise CommandError if bytes are not a valid image (Pillow-verified)."""
    import io
    ...
    try:
        ...
        raise CommandError(f"Generated bytes are not a valid image{label}: {exc}") from exc


def _image_extension(image_bytes: bytes) -> str:
    """Return the correct file extension for image bytes based on actual format.

    OpenAI image models (e.g. gpt-image-1) return PNG by default regardless of
    the filename used when saving.  Always detecting the real format prevents a
    validator mismatch between the stored extension and the file magic bytes.
    """
    import io
        ...
        for dish_name in dish_names:
            try:
                ...
                if generate_image:
                    ...
                    raw_hero = visual_plan.get("hero_image", {}) if visual_plan else {}
                    # Validate planner output: hero_plan must be a dict, not a string/list
                    hero_plan = raw_hero if isinstance(raw_hero, dict) else {}
                    ...
                    try:
                        ...
                        _validate_image_bytes(image_bytes, context="hero image")
                        ext = _image_extension(image_bytes)
                        recipe.hero_image.save(f"cover-{recipe.slug[:40]}{ext}", ContentFile(image_bytes), save=False)
                ...
                if len(dish_names) > 1:
                    self.stderr.write(self.style.ERROR(f'Failed "{dish_name}": {exc}'))
                    continue
                raise
            except Exception as exc:
                logger.error("generate_recipe: unexpected failure for %r: %s", dish_name, exc, exc_info=True)
                if len(dish_names) > 1:
                    self.stderr.write(self.style.ERROR(f'Failed "{dish_name}": {exc}'))
                    continue
                raise
