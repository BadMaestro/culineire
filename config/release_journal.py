RELEASE_JOURNAL = [
    {
        ...
        "date": "2026-08-04",
        "commit": "",
        "title": "A07 - the octagon was never the wrong shape, the camera was",
        "section": "Chef Battles / Arena",
        "summary": "A07 was written to widen and flatten the octagon by 64 percent. It needed neither. The Design Template's floor is a 1120x1120 SQUARE with no scaling in its transform matrix - the same square as this SVG's 1100x1100 viewBox - and its aspect of 2.375 comes entirely from its camera: rotateX(57deg), perspective 1600px, both origins centred, against our 42deg, 1500px and an origin at 62 percent. Applied to our own SVG on the live page, that camera projects the square to 1585.4 x 667.5: the reference floor to the decimal, with the multiplier set from the element's size rather than from the target, so it is a reproduction and not a fit. The Owner replaced his own 42deg restore of 2026-07-27 for this. A06's headline delta was also an artefact of comparing unlike things - their whole floor square against our RANK CELLS ONLY, without moat, VIP, stands or outer band; like for like it was 1.467 against 2.375, and the direction was right while the size of the gap was not. TWO THINGS THE CHANGE HAD TO CARRY, and both were landmines rather than work. The camera is declared in arena_deck_polish.css SEVEN times, one of them !important, so the comment in arena_render.css calling itself 'the only place the camera is declared' was false; it cost a measurement pass that silently read back unchanged, and the numbers from it were nearly reported as '57deg does not help'. Both files now carry 57deg and each says the other exists. And fitScene measured the rank floor while its own comment instructed measuring the stands: at 42deg the two agreed closely enough, at 57deg they did not, and fitting the rank floor to 0.64 of the frame put the full scene at 2195px inside a 1908px frame - 287px of stands hanging out of each side. The fit now measures the SVG's own box and uses the Design Template's own pads (0.8257 of canvas width, 0.618 of height). It also had to stop counting passes: --arena-shift-* translates the element BEFORE the rotation, so the centring pass moves the scene through the perspective frustum and changes its projected height - measured, 667.5 to 609.6 - which couples fit and centre. A fixed three passes landed wherever the oscillation happened to be, 899 then 1915 then 1447px of width, so the scale would have differed on every load. Correction is now damped by half and stops when it asks for under 0.5 percent: six passes at 1920, seven at 1280, and damped and undamped runs converge on the same fit, which is what says the fixed point belongs to the scene and not to the damping. MEASURED AFTER, on production: at 1920 the scene is 1576.8 x 630.3, aspect 2.502, width within 8.6px of the reference; at 1280 it is 1039.4 x 453.9, aspect 2.29; the rank floor reads 1.852 and 1.847, so the shape holds across viewports. Inside the frame at both, scrollWidth 1910 of 1920 and 1270 of 1280 - nothing overflows. The aspect lands at 2.502 rather than 2.375 because our scene carries stands and balconies the reference does not, and that is left stated rather than tuned away. Ring geometry, cell counts, floor colours and the seat contract are untouched; billboardFaces needed no change because it measures each face rather than carrying a hardcoded cos(42). Two pre-existing failures in chef_battle are recorded and are not from this card - event log ordering and a 302/404 in battle readiness both fail identically on the unmodified base.",
    },
    {
        "version": "2.5.791",
        "date": "2026-08-04",
