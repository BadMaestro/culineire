...
| AC-BTN | Arena chat | **One button family, and a lighter panel outline.** Every button in the chat is to belong to the site's colour scheme, and `.arena-chat`'s outline is to be lighter. | Bolt | AC-P2 | **DONE v2.5.1375 - and the row above this one was wrong for two days, which is the part worth reading.** It said NEXT and unstarted. It had in fact SHIPPED TWICE on 2026-08-26: `fa1a29eb` v2.5.1308 made one filled bronze family of every listed control, and `031b8500` v2.5.1317 - the same day, a separate 44-part visual brief with a mock-up - partly undid it for the DESKTOP ONLY, because that brief said in terms **DO NOT MODIFY MOBILE CHAT FILES**. Gating the second on `min-width: 901px` obeyed the instruction and left two chats standing: a phone rendered the filled family, a desktop rendered half of it corrected. Nobody wrote that down and the board still read NEXT, so I told the Owner the card was unstarted and he agreed to it on that basis. Corrected here rather than quietly. **THE OWNER CLOSED THE SPLIT on 2026-08-28** - the buttons are brown and do not belong to the site's scheme, on the desktop AND the phone - and named the standard: *«наша единая цветовая схема сайта не бронзовая - идеальный её пример это виджет арены»*, asked and answered as `site-battle-widget`, the floating card. **ITS HEX VALUES ARE NOT COPIED AND THAT IS DELIBERATE**: that widget is a dark glass card and the chat is light parchment, so importing `#f8d28a` would have been a third palette, not one scheme. What transferred is its behaviour, which its own primary control states plainly - `.site-battle-widget .btn-primary` is a 1px translucent edge over a 10% wash, so even the main button there is not filled, and gold appears only under the pointer. Translated into the light palette through the ratio the Arena already uses for `.arena-command-tile` (22% of `--brand`), so nothing is invented. **What shipped**: `--chat-btn-fill/ink/edge/shadow` now describe the RESTING state - transparent fill, `--ink-soft`, a 22% `--brand` edge, no shadow - with a second `-on` trio carrying the bronze for hover and focus; every `border: 0` in the family became that edge; the underline tabs, the quieter Pinned Rules and the neutral composer icons were promoted off their desktop gate and **the gate itself is deleted**, so no width changes the chat's buttons; the send button keeps its 2.3rem weight and loses its gradient; the media viewer's Report loses its fill. **Zero bronze gradients remain in the file.** The chat's own frame is finally lighter too - `.arena-chat` overrides the seven-panel 55% bronze with the same 22% `--brand`, scoped to the chat alone, so the floor, the status card, the ladder, the metrics, the gifts and the phase rail are untouched. He had said *«пока не меняй»* on that half on 2026-08-26 and opened it on 2026-08-28. **Not touched**: the four chat themes (ivory/dark/copper/midnight) each redeclare `--accent-bronze` on `.arena-chat[data-theme]` and are a deliberate per-reader palette set from `arena_chat.js`, not the source of the complaint. |
| AC-STK | Arena economy | **The Arena's second purchase stage.** Part A: the 13 chat stickers become paid goods - 10 tokens each, 100 the pack, a Stickers section in the shop, ownership keyed to `RecipeAuthor`, and a server-side check on `arena_chat_send` without which the purchase means nothing. Part B: buying an artifact for yourself at shelf price into your own chest. Part C: gifting an artifact to a chef BEFORE a battle free of delivery, and sending one you already own into a running battle for the delivery half. | GreenBear | AC-BTN | **HANDED OVER by Bolt on the Carpet, #3548, 35k chars.** The Owner gave GreenBear the whole card, not only the schema. THE ECONOMY WAS CORRECTED IN THE SAME BREATH: he ruled 2026-08-27 that there is no ban on buying artifacts and never was - four lines in `tz_main.md` and `artifact_3_models_rules.md` said otherwise for months while the PUBLIC rules page described the opposite, and they misled an agent into telling him his own product was forbidden. Those lines are struck and the four acquisition routes recorded in their place. His standing instruction now: the prompts and the spec change faster than anyone records them, so when something new is not written down, WRITE IT DOWN YOURSELF. Four traps measured before any code and carried in the brief: the five `LEGACY_STICKERS` aliases make the enforcement set 18 tokens and not 13 (a hand-typed `:fired:` draws the paid SEARED painting); the ten custom emoji share the `:token:` syntax and must pass untouched; `arena_chat_send` must NOT gain `chef_battle_guard` (it is in `UNGUARDED_BY_DESIGN`); and `getattr(user, "recipe_author")` is a dead accessor that always returns None. **AUTHORISED AND PART A IS DONE, v2.5.1373.** He gave the deploy word for the whole card, A+B+C, on 2026-08-27 - the section 8 exclusion on migrations and the token-spend path is lifted for this work and only this work. He also ruled the one open product question in the brief: **legendary artifacts stay prize-only and cannot be bought**, so the refusal already in `send_battle_artifact` is extended to every new path rather than relaxed. **Part A shipped**: `StickerPack`/`StickerItem`/`ChefSticker` (migrations 0112, 0113 - 0113 reverses plainly per #3549), `buy_sticker`/`buy_sticker_pack` with the unique constraint as the idempotency, the three guarded shop URLs, the send-path check over the full **18-token** set, the picker's locked tiles failing CLOSED, and the grant tool as a **moderation panel item and not Django Admin** (#3550). 39 new tests green on PostgreSQL. **PART B SHIPPED, v2.5.1376** - `buy_artifact()` at the shelf price with no doubling, `source=PURCHASED` passed explicitly (it is also the field default, and 421 production rows wear it from a seed that never chose it), and the first producer this project has ever had for `TxType.ARTIFACT_BOUGHT` and `LedgerEvent.ARTIFACT_PURCHASED` - both defined in migration 0007 and written by nothing since. Legendary refused in the same words as the battle-gift path, pinned by a test that counts the sentence. Duplicates allowed, as everywhere else. 17 new tests, 89 green. **TWO DEFECTS THE OWNER FOUND ON HIS OWN SCREEN, FIXED v2.5.1379.** (1) Every sticker was locked for every reader including him: the view returned a JSON *string* and the template printed it, so Django autoescaped the quotes, JSON.parse threw, and the client's fail-closed branch locked everything. The Python and the JavaScript were each tested and the SEAM between them was not; it is `json_script` now, with five tests that render the template and parse the result. (2) The shelf was built as a separate PAGE when he asked for a SECTION of the artifact shop, and the shop did not link to it - so he followed the link and found nothing. It is a section of `artifact_gallery.html` now, the filter sidebar carries it, and the separate page is deleted. The locked-tile treatment (grayscale plus a bronze badge over his own artwork) is gone too. **FOUR MORE OWNER RULINGS, v2.5.1385.** (1) An unbought sticker is NOT IN THE PICKER at all - not dimmed, not locked, not in full colour: a tile you cannot use is rubbish in the chat window, because people think they own it, click, and nothing happens. The whole locked-tile idea is deleted, class, CSS and tests; a reader who owns none gets ONE LINK to the Stickers section. The send-path check is untouched - the picker was never the gate. (2) **NOOOO! is in the pack and sold separately for 100T** - the pack is TWELVE for 100 and only with the thirteenth is the collection complete (`StickerItem.sold_separately`, migrations 0114/0115). That settles the shop window by itself: his contact sheet shows twelve and the pack now sells twelve, so nothing was drawn onto his artwork. The sheet is placed at the head of the section. (3) Tile size left unchanged, on his instruction, against the brief. (4) **PART B FINISHED and UNFROZEN on his word**: everything the specification named shipped in v2.5.1376, and what was missing is what it never said - the page charged tokens without showing the balance, and said "it is in your chest" with no link to the chest. **PART C SHIPPED, v2.5.1388 - AC-STK IS COMPLETE.** C1: gifting before a battle costs the shelf price and NO delivery, and lands as ordinary property - not locked, not forced, not expiring, proven by a test that ends a battle and finds it still there. It writes `Source.GIFTED`, the third dead constant this card gave a producer. C2: sending one you already own costs the delivery half, and the ROW MOVES rather than being copied - a copy would mint an artifact out of nothing, and a test counts the table before and after. Both the battle row and the artifact row are locked and re-read under their own locks (F37's lesson), with five tests on the refusals. Legendary refused on both; that guard now asserts the WORDING across every path rather than a count somebody has to bump. 21 new tests, 112 green. All four of the Owner's acquisition routes now exist in code. **A HOLE HE FOUND HIMSELF, CLOSED v2.5.1519.** The ownership check guards the TOKEN and knows nothing about a picture, and the 13 sticker files are ordinary static assets - so the artwork could be downloaded, or copied out of somebody else's message, and uploaded back as an ordinary chat image. The chat upload path now compares every incoming picture against the shop's own artwork by difference hash: all 13 caught after halving and re-encoding, screenshots caught with 0-30% of page around them, 200 unrelated site images refused none, and the two closest stickers are 15 bits apart against a threshold of 8. Refused for owners too - they have the picker. A crop, a mirror or a photograph of the screen still passes, and no browser can block a screenshot: that was researched and reported to him separately. |
