> **СВЕРЕНО С КОДОМ 11.08.2026 — GreenBear, полный акт:
> `docs/chef_battle/AUDIT_2026-08-10_GREENBEAR.md`, раздел 8.**
>
> Правило Владельца от 10.08.2026: «код уточнялся по факту… я склонен больше
> доверять коду — ТЗ и правки нужно адаптировать». У правила есть граница: оно
> решает СПОР, где решение — это код; оно не покрывает правило, которое код
> никогда не строил, потому что отсутствие — не решение.
>
...
>
> **Не построено — твоё решение (раздел 8.3 акта):** разделение Culinary
> Reputation и Battle Rating (§9) — репутация двигается ТОЛЬКО по боям, ни
> рецепт, ни статья, ни лайк её не меняют, хотя ТЗ отдельно объясняет, зачем
> эти две лестницы разные; сила соперника и убывающая отдача за повторного
> соперника (§9, §16); `balance_after` в леджере ходов (§17.7); wins/losses/
> streak в SeasonStanding (§17.13); `crown_rule` и `reward_rules_json` у сезона
> (§17.12); связь боя с сезоном (§17.3); страницы history, season/<slug>,
> crown-holder и отдельная страница разбора голосов (§18).
